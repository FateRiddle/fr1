const a = {
  propsSchema: {
    properties: {
      crowd: {
        properties: {
          upfIdType: {
            title: '圈人平台ID类型',
            description: '',
            type: 'string',
            enum: ['1', '2'],
            enumNames: ['白名单', '黑名单'],
          },
          upfIdList: { title: '圈人平台ID', description: '', type: 'string' },
          realNameChecked: {
            title: '实名可见',
            description: '',
            type: 'boolean',
          },
          realNameIdentityChecked: {
            title: '身份证认证',
            description: '',
            type: 'boolean',
          },
          vipLevels: {
            title: '飞猪会员身份',
            description: '',
            type: 'array',
            items: { type: 'string' },
            enum: ['1', '101', '2', '3'],
            enumNames: ['F1(原F1)', 'F2(新F1+)', 'F3(原F2)', 'F4(原F3)'],
          },
          bbVipCheck: { title: '88vip认证', description: '', type: 'boolean' },
          dreamCouponLotteryCheck: {
            title: '99梦想券抽奖专用',
            description: '',
            type: 'boolean',
          },
          saveMoneyCardCheck: {
            title: '飞猪省钱卡用户',
            description: '',
            type: 'boolean',
          },
        },
        type: 'object',
        required: [],
        title: '人群限制',
        name: 'crowd',
      },
      limit: {
        properties: {
          totalLimitCount: {
            title: '库存总数',
            description: '保底库存最少X个',
            type: 'string',
            pattern: '^([0-9])+$',
          },
          addAllCount: {
            title: '已补充总库存',
            description: '',
            type: 'string',
          },
          totalLimitId: {
            title: '库存限制ID',
            description: '',
            type: 'string',
            pattern: '^([1-9]\\d*|0)$',
          },
          surplusCount: {
            title: '剩余有效库存',
            description: '',
            type: 'string',
          },
          noActiveAllCount: {
            title: '未生效总库存',
            description: '',
            type: 'string',
          },
          userLimitTotal: {
            title: '单用户总参与次数',
            description: '用户在该活动总共可领取数量',
            type: 'string',
            pattern: '^([1-9]\\d*)$',
          },
          showAdvancedSetting: {
            title: '高级配置展开显示（收起不影响功能）',
            description: '',
            type: 'boolean',
          },
          userLimitDay: {
            title: '单用户每天参与次数',
            description: '用户在该活动每天可领取数量',
            type: 'string',
            pattern: '^([1-9]\\d*)$',
          },
          userLimitWeek: {
            title: '单用户每周参与次数',
            description: '用户在该活动每周可领取数量',
            type: 'string',
            pattern: '^([1-9]\\d*)$',
          },
          userLimitNaturalMonth: {
            title: '单用户自然月参与次数',
            description: '按自然月限制，非30天周期',
            type: 'string',
            pattern: '^([1-9]\\d*)$',
          },
          limitDay: {
            title: '活动每天可被参与的次数（慎重）',
            description: '该活动每天可被领取的数量',
            type: 'string',
            pattern: '^([1-9]\\d*)$',
          },
          userWinningLimitTotal: {
            title: '单用户总可中奖次数',
            description: '',
            type: 'string',
            pattern: '^([1-9]\\d*|0)$',
          },
          userWinningLimitDay: {
            title: '单用户每天可中奖次数',
            description: '',
            type: 'string',
            pattern: '^([1-9]\\d*|0)$',
          },
          userWinningLimitWeek: {
            title: '单用户每周可中奖次数',
            description: '',
            type: 'string',
            pattern: '^([1-9]\\d*|0)$',
          },
          latour2SendFliggyPackage: {
            title: '拉菲发飞猪礼包',
            description: '',
            type: 'string',
            enum: ['0', '1'],
            enumNames: ['否', '是'],
          },
          hasAddItems: {
            title: '是否添加了添加库存的item',
            description: '',
            type: 'string',
            default: '1',
          },
          groupAddLimitText: {
            title: '定时添加活动库存',
            description: '',
            type: 'string',
          },
          incrInventoryItems: {
            minItems: 0,
            maxItems: 30,
            type: 'array',
            items: {
              properties: {
                addTime: { title: '补充时间', description: '', type: 'string' },
                addCount: {
                  title: '补充数量',
                  description: '',
                  type: 'string',
                  pattern: '^([1-9]\\d*|0)$',
                },
                validCount: {
                  title: '已生效数量',
                  description: '',
                  type: 'string',
                  pattern: '^([1-9]\\d*|0)$',
                },
                taskId: {
                  title: '执行添加库存任务的Id',
                  description: '',
                  type: 'string',
                },
                itemId: {
                  title: '补充库存的记录Id',
                  description: '',
                  type: 'string',
                },
              },
              type: 'object',
              required: ['addTime', 'addCount'],
              title: '设置定时补充库存',
              name: 'limitAddItem',
            },
          },
        },
        type: 'object',
        required: [
          'totalLimitCount',
          'userLimitTotal',
          'userWinningLimitTotal',
        ],
        title: '参与次数限制',
        name: 'limit',
      },
      prizeGroupConfigList: {
        properties: {
          prizeGroupList: {
            minItems: 1,
            maxItems: 8,
            uniqueItems: 'name',
            type: 'array',
            items: {
              properties: {
                name: {
                  title: '奖品组名称',
                  description: '',
                  type: 'string',
                  maxLength: 16,
                  minLength: 1,
                },
                lotteryProbability: {
                  title: '奖品组中奖概率',
                  description: '后面输入框输小数',
                  type: 'number',
                  step: '{{1/formData.lotteryConfig.digit}}',
                  max: 1,
                },
                groupId: {
                  title: '奖品组ID',
                  description: '',
                  type: 'string',
                  maxLength: 16,
                  minLength: 1,
                },
                groupIdVO: {
                  title: '奖品组ID',
                  description: '',
                  type: 'number',
                  maxLength: 16,
                  minLength: 1,
                },
                inventoryId: {
                  title: '库存ID',
                  description: '',
                  type: 'string',
                  maxLength: 16,
                  minLength: 1,
                },
                inventoryLimit: {
                  title: '库存限制',
                  description: '',
                  type: 'string',
                  maxLength: 16,
                  minLength: 1,
                  pattern: '^([0-9])+$',
                },
                groupAddAllCount: {
                  title: '奖品组已补充总库存',
                  description: '',
                  type: 'string',
                },
                surplusCount: {
                  title: '剩余库存',
                  description: '',
                  type: 'string',
                  maxLength: 16,
                  minLength: 1,
                },
                groupNoActiveAllCount: {
                  title: '奖品组未生效总库存',
                  description: '',
                  type: 'string',
                },
                userGroupWinningLimitTotal: {
                  title: '该奖品组单用户可中次数',
                  description: '',
                  type: 'string',
                  maxLength: 16,
                  minLength: 1,
                },
                groupShowAdvancedSetting: {
                  title: '库存高级配置展开显示（收起不影响功能）',
                  description: '',
                  type: 'boolean',
                },
                userGroupWinningLimitDay: {
                  title: '该奖品组单用户每天可中次数',
                  description: '',
                  type: 'string',
                  maxLength: 16,
                  minLength: 1,
                },
                userGroupWinningLimitWeek: {
                  title: '该奖品组单用户每周可中次数',
                  description: '',
                  type: 'string',
                  maxLength: 16,
                  minLength: 1,
                },
                lotteryApplyMsg: {
                  title: '中奖提示文案',
                  description: '',
                  type: 'string',
                },
                groupAddLimitText: {
                  title: '定时添加库存',
                  description: '',
                  type: 'string',
                },
                groupHasAddItems: {
                  title: '奖品组是否添加了添加库存的item',
                  description: '',
                  type: 'string',
                  default: '1',
                },
                groupIncrInventoryItems: {
                  minItems: 0,
                  maxItems: 30,
                  type: 'array',
                  items: {
                    properties: {
                      groupAddTime: {
                        title: '补充时间',
                        description: '',
                        type: 'string',
                      },
                      groupAddCount: {
                        title: '补充数量',
                        description: '',
                        type: 'string',
                        pattern: '^([1-9]\\d*|0)$',
                      },
                      groupValidCount: {
                        title: '已生效数量',
                        description: '',
                        type: 'string',
                        pattern: '^([1-9]\\d*|0)$',
                      },
                      groupTaskId: {
                        title: '执行添加库存任务的Id',
                        description: '',
                        type: 'string',
                      },
                      groupItemId: {
                        title: '补充库存的记录Id',
                        description: '',
                        type: 'string',
                      },
                    },
                    type: 'object',
                    required: ['groupAddTime', 'groupAddCount'],
                    title: '设置定时补充库存',
                    name: 'limitGroupAddItem',
                  },
                },
                prizeConfig: {
                  title: '奖品配置',
                  description: '',
                  type: 'string',
                },
                prizeList: {
                  minItems: 1,
                  maxItems: 8,
                  type: 'array',
                  items: {
                    properties: {
                      couponName: {
                        title: '奖品名称',
                        description: '',
                        type: 'string',
                        maxLength: 11,
                      },
                      prizeType: {
                        title: '奖品类型',
                        description: '',
                        type: 'string',
                        enum: [
                          'FLIGGY_NEW_COUPON',
                          'BENEFIT_CENTER',
                          'FLIGGY_CARD_PACKAGE',
                          'EMPTY_PRIZE',
                          'FLIGGY_F2_VIP',
                          'TAOBAO_COUPON',
                          'VIRTUAL_BONUS_POINTS',
                          'PUSH_MESSAGE',
                          'ALITRIP_UMP_COMMON_COUPON',
                          'ALITRIP_COUPON',
                          'FLIGGY_MILEAGE',
                          'ALIPAY_SHOP_BONUS',
                          'ALIPAY_OVERSEA_COUPON',
                          'FLIGGY_RIGHT',
                          'ALIPAY318_FLIGGY_NEW_COUPON',
                          'THURSDAY_DOUBLE_MILES',
                          'WRITE_OFF_CERTIFICATE_COUPON',
                          'CERTIFICATE_COUPON_PACKAGE',
                          'FLIGGY_F1_VIP',
                          'LATOUR2_BENEFIT',
                          'ALIPAY_OFFLINE_VOUCHER',
                          'ALIPAY_RED_PACKED',
                          'DREAM_COUPON_VIRTUAL_BONUS_POINTS',
                          'RANDOM_AMT_ALIPAY_RED_PACKED_4_DREAM_COUPON',
                          'RANDOM_AMT_FLIGGY_RED_PACKED_4_DREAM_COUPON',
                          'FLIGGY_TCCP_RIGHT',
                          'CYCLE_FLIGGY_NEW_COUPON',
                          'FLIGGY_VOUCHER',
                          'SUPER_VIP_CUSTOM_FREE_TICKET',
                          'SUPER_VIP_CUSTOM_FEIZHU',
                          'SUPER_VIP_CUSTOM_TRAIN_NO_WORRIES_BACK',
                          'SUPER_VIP_CUSTOM_MULTI_MILES',
                          'REDEEM_CODE',
                        ],
                        enumNames: [
                          '飞猪新卡券(除随机面额优惠券)',
                          '权益中心',
                          '飞猪玩乐卡',
                          '空奖品',
                          '飞猪F3(原F2)会员',
                          '淘宝UMP优惠券',
                          '虚拟积分(抽奖凭证)',
                          'push奖品',
                          '航旅平台券',
                          '航旅平台老券(含020券)',
                          '飞猪里程',
                          '支付宝到店红包',
                          '支付宝境外线下券',
                          '飞猪会员权益资源',
                          '支付宝318专用飞猪新卡券',
                          '周四会员福利日里程翻倍',
                          '核销凭证券',
                          '新玩法凭证类产品',
                          '飞猪F1会员',
                          '拉菲2权益',
                          '支付宝线下券',
                          '支付宝红包',
                          '梦想券专用虚拟积分',
                          '梦想券专用支付宝红包',
                          '梦想券专用飞猪红包',
                          '履约链路发放飞猪权益资源',
                          '周期有效的飞猪新卡券',
                          '飞猪资格凭证',
                          '定制超级会员4张0.1元景区奖品',
                          '定制超级会员菲住奖品',
                          '定制超级会员12张火车票无忧退奖品',
                          '定制超级会员里程翻倍奖品',
                          '兑换码',
                        ],
                      },
                      fliggyNewCouponType: {
                        title: '卡券类型',
                        description: '',
                        type: 'string',
                        enum: ['288', '283', '287', '285', '289'],
                        enumNames: [
                          '平台出资优惠券（9）',
                          '行业商家优惠券（4）',
                          '日历房酒店纯平台出资优惠券（6）',
                          '飞猪红包（10）',
                          '资格券（0）',
                        ],
                      },
                      rightsId: {
                        title: '模板Id',
                        description: '',
                        type: 'string',
                      },
                      channel: {
                        title: '渠道',
                        description: '',
                        type: 'string',
                      },
                      source: {
                        title: '卡券发放sourceId',
                        description: '',
                        type: 'string',
                      },
                      pushBizCode: {
                        title: 'bizCode',
                        description: '',
                        type: 'string',
                      },
                      actionCode: {
                        title: 'actionCode',
                        description: '',
                        type: 'string',
                      },
                      pushExtInfo: {
                        title: 'pushExtInfo',
                        description: '',
                        type: 'string',
                      },
                      virtualValue: {
                        title: '虚拟积分发放数量(1代表抽奖一次)',
                        description: '',
                        type: 'number',
                      },
                      durationDays: {
                        title: '会员期限',
                        description: '',
                        type: 'string',
                        default: '365',
                      },
                      vipActiveChannel: {
                        title: 'F1会员激活渠道',
                        description: '',
                        type: 'string',
                      },
                      businessLine: {
                        title: '业务线',
                        description: '',
                        type: 'string',
                        enum: [
                          'TRIP_PUBLIC',
                          'TRIP_DOMESTIC_AIR',
                          'TRIP_INTERNATIONAL_AIR',
                          'TRIP_HOTEL',
                          'TRIP_HOTEL_PKG',
                          'TRIP_HOLIDAY',
                          'TRIP_ENTRANCE',
                          'TRIP_TRAIN',
                          'TRIP_CAR_SERVICE',
                          'TRIP_BUS',
                          'TRIP_MEMBERSHIP',
                          'TRIP_MEETING_PLACE',
                        ],
                        enumNames: [
                          '公共',
                          '国内机票',
                          '国际机票',
                          '酒店日历房',
                          '酒店套餐',
                          '度假',
                          '门票',
                          '火车票',
                          '用车',
                          '汽车票',
                          '会员',
                          '主会场',
                        ],
                      },
                      couponDesc: {
                        title: '奖品描述',
                        description: '',
                        type: 'string',
                        maxLength: 11,
                      },
                      couponDescPic: {
                        title: '奖品logo',
                        description: '',
                        type: 'string',
                      },
                      couponUseUrl: {
                        title: '奖品去使用链接(无线)',
                        description: '',
                        type: 'string',
                      },
                      hidden: {
                        title:
                          '卡券不在手淘、飞猪卡券包露出（支付宝是否露出在优惠配置时决定）',
                        description: '',
                        type: 'boolean',
                      },
                      resourceId: {
                        title: '奖品资源Id',
                        description: '',
                        type: 'string',
                      },
                      rightsSellerType: {
                        title: '权益中心类型',
                        description: '',
                        type: 'string',
                        enum: ['135', '137', '138', '139', '141', '142', '145'],
                        enumNames: [
                          '大麦',
                          '虾米',
                          '银泰券',
                          '口碑',
                          '饿了么',
                          '淘票票',
                          '优酷',
                        ],
                      },
                      exchangeTotal: {
                        title: '兑换次数上限',
                        description: '',
                        type: 'number',
                        min: 1,
                      },
                      taobaoCouponInstanceSource: {
                        title: '卡券传播渠道',
                        description: '',
                        type: 'number',
                      },
                      taobaoCouponQuerySource: {
                        title: '卡券领取来源',
                        description: '',
                        type: 'number',
                      },
                      taobaoCouponSpreadId: {
                        title: '红包/卡券 Spread Id',
                        description: '',
                        type: 'number',
                      },
                      taobaoCouponSupplierId: {
                        title: '卖家uid',
                        description: '',
                        type: 'number',
                      },
                      taobaoCouponUUID: {
                        title: '红包/卡券 uuid',
                        description: '',
                        type: 'string',
                      },
                      fliggyNewCouponCycleType: {
                        title: '卡券有效期模式',
                        description: '',
                        type: 'string',
                        enum: ['1', '2', '3'],
                        enumNames: ['自然周', '自然月', '自然年'],
                      },
                      mileageCount: {
                        title: '发放里程数',
                        description: '',
                        type: 'string',
                        pattern: '^([1-9]\\d*|0)$',
                      },
                      mileageMemo: {
                        title: '发放原因文案',
                        description: '',
                        type: 'string',
                      },
                      showAdvancedSetting: {
                        title: '高级配置',
                        description: '',
                        type: 'boolean',
                      },
                      faceValue: {
                        title: '面额',
                        description: '',
                        type: 'string',
                        enum: ['1', '2', '3', '4', '5'],
                        enumNames: [
                          '固定金额',
                          '固定折扣',
                          '减至',
                          '纯文案',
                          '自定义前中后缀',
                        ],
                      },
                      fixedAmountBefore: {
                        title: '',
                        description: '',
                        type: 'string',
                      },
                      fixedAmountAfter: {
                        title: '',
                        description: '',
                        type: 'string',
                      },
                      discount: { title: '', description: '', type: 'string' },
                      reduce: { title: '', description: '', type: 'string' },
                      faceValuePurePrefixText: {
                        title: '前缀',
                        description: '',
                        type: 'string',
                      },
                      faceValuePureInfixText: {
                        title: '中缀',
                        description: '',
                        type: 'string',
                      },
                      faceValuePureText: {
                        title: "{{rootValue.faceValue=='5'?'后缀':''}}",
                        description: '',
                        type: 'string',
                      },
                      effectiveTimeType: {
                        title: '使用有效期',
                        description: '',
                        type: 'string',
                        enum: ['1', '2', '3', '4'],
                        enumNames: [
                          '固定时间',
                          '自领取之时起',
                          '自领取之日起',
                          '纯文案',
                        ],
                      },
                      beginTime: {
                        title: '开始时间',
                        description: '',
                        type: 'string',
                      },
                      endTime: {
                        title: '结束时间',
                        description: '',
                        type: 'string',
                      },
                      effectiveMin: {
                        title: '',
                        description: '',
                        type: 'string',
                        pattern: '^[0-9]+$',
                      },
                      effectiveDay: {
                        title: '',
                        description: '',
                        type: 'string',
                        pattern: '^[0-9]+$',
                      },
                      effectTimePureText: {
                        title: '',
                        description: '',
                        type: 'string',
                      },
                      toUseUrl4Pc: {
                        title: '奖品去使用链接(PC)',
                        description: '',
                        type: 'string',
                      },
                      toUseUrl4Wx: {
                        title: '奖品去使用链接(WX)',
                        description: '',
                        type: 'string',
                      },
                      tag: { title: 'tag', description: '', type: 'string' },
                      count: { title: '数量', description: '', type: 'string' },
                      desc: { title: '说明', description: '', type: 'string' },
                      url: { title: 'URL', description: '', type: 'string' },
                      partnerCode: {
                        title: 'partnerCode',
                        description: '',
                        type: 'string',
                      },
                      amount: {
                        title: '数值',
                        description: '支付宝红包的单位为分，填100等于1元',
                        type: 'string',
                      },
                      cashPoolId: {
                        title: '资金池ID',
                        description: '',
                        type: 'string',
                      },
                      cashPoolUserId: {
                        title: '资金池所属用户ID',
                        description: '',
                        type: 'string',
                      },
                      subBusinessType: {
                        title: '子业务类型',
                        description: '',
                        type: 'string',
                      },
                      priceReduce: {
                        title: '价值(面额)(元):扣减资源数',
                        description: '',
                        type: 'string',
                      },
                      securityCode: {
                        title: '安全码',
                        description: '',
                        type: 'string',
                      },
                      topPrice: {
                        title: '最高价值(面额)(元)',
                        description: '',
                        type: 'string',
                      },
                      stgCode: {
                        title: '投放Id',
                        description: '',
                        type: 'string',
                      },
                      multiple: {
                        title: '里程翻倍倍数',
                        description: '',
                        type: 'string',
                      },
                      maxSendMiles: {
                        title: '最大里程发放数量',
                        description: '',
                        type: 'string',
                      },
                      subActMaxSendMiles: {
                        title: '该活动最大里程发放数量',
                        description: '',
                        type: 'string',
                      },
                      dreamCouponMaxAmount: {
                        title: '最大优惠红包金额',
                        description: '',
                        type: 'number',
                      },
                      dreamCouponPrizeType: {
                        title: '对应发放的红包类型',
                        description: '',
                        type: 'string',
                        enum: ['1', '2'],
                        enumNames: ['支付宝红包', '飞猪红包'],
                      },
                      dreamCouponMatchVirtualScoreActivityId: {
                        title: '对应匹配的虚拟积分活动id',
                        description: '',
                        type: 'number',
                      },
                    },
                    type: 'object',
                    required: [
                      'couponName',
                      'prizeType',
                      'fliggyNewCouponType',
                      'rightsId',
                      'channel',
                      'source',
                      'pushBizCode',
                      'actionCode',
                      'pushExtInfo',
                      'rightsSellerType',
                      'taobaoCouponInstanceSource',
                      'taobaoCouponQuerySource',
                      'taobaoCouponSpreadId',
                      'taobaoCouponSupplierId',
                      'taobaoCouponUUID',
                      'fliggyNewCouponCycleType',
                      'mileageCount',
                      'effectiveMin',
                      'effectiveDay',
                      'tag',
                      'count',
                      'desc',
                      'url',
                      'partnerCode',
                      'amount',
                      'cashPoolId',
                      'cashPoolUserId',
                      'subBusinessType',
                      'priceReduce',
                      'securityCode',
                      'topPrice',
                      'stgCode',
                      'dreamCouponMaxAmount',
                      'dreamCouponPrizeType',
                      'dreamCouponMatchVirtualScoreActivityId',
                    ],
                    title: '卡券配置',
                    name: 'prizeTotalConfig',
                  },
                },
              },
              type: 'object',
              required: [
                'name',
                'lotteryProbability',
                'groupId',
                'inventoryId',
                'inventoryLimit',
              ],
              name: 'prizeGroupConfig',
            },
          },
        },
        type: 'object',
        required: [],
        title: '奖品组配置列表',
        name: 'prizeGroupConfigList',
      },
      prizeSendRuleList: {
        properties: {
          prizeSendRuleList: {
            minItems: 0,
            maxItems: 8,
            type: 'array',
            items: {
              properties: {
                ruleDesc: {
                  title: '规则描述',
                  description: '',
                  type: 'string',
                  maxLength: 12,
                  minLength: 1,
                },
                unMatchNotice: {
                  title: '未命中提示',
                  description: '',
                  type: 'string',
                  maxLength: 36,
                  minLength: 1,
                },
                strategyId: {
                  title: '规则策略Id',
                  description: '',
                  type: 'string',
                },
                ruleConditionCfg: {
                  minItems: 1,
                  maxItems: 8,
                  type: 'array',
                  items: {
                    properties: {
                      conditionName: {
                        title: '',
                        description: '',
                        type: 'string',
                        enum: [
                          '3002',
                          '3004',
                          '3008',
                          '4001',
                          '5001',
                          '9001',
                          '10001',
                          '20002',
                          '24001',
                          '24004',
                          '26001',
                          '27001',
                          '27002',
                          '35001',
                          '35002',
                          '36001',
                          '59001',
                          '59002',
                          '60001',
                          '97001',
                          '115001',
                          '116001',
                          '117001',
                          '131001',
                          '134001',
                          '169001',
                          '170002',
                          '173001',
                          '178001',
                          '180001',
                          '181001',
                          '182001',
                          '182002',
                          '183001',
                          '184001',
                          '185001',
                          '186001',
                          '187001',
                          '188001',
                          '189001',
                          '194001',
                          '200001',
                          '201001',
                          '207001',
                          '208001',
                          '212001',
                          '213001',
                          '215001',
                          '216001',
                          '219001',
                          '220001',
                          '221001',
                          '222001',
                          '223001',
                          '224001',
                          '225001',
                          '226001',
                          '229001',
                          '230002',
                          '231001',
                          '232001',
                          '234001',
                          '234002',
                          '235001',
                          '235002',
                          '236001',
                          '237001',
                          '238001',
                          '239001',
                          '240001',
                          '241001',
                          '244001',
                          '245001',
                          '250001',
                          '251001',
                          '251002',
                          '252001',
                          '253001',
                          '255001',
                          '257001',
                          '258001',
                          '259001',
                          '261001',
                          '271001',
                          '273001',
                          '281001',
                          '285001',
                          '286001',
                          '289001',
                        ],
                        enumNames: [
                          '用户是否拥有足够里程值',
                          '是否有真实姓名',
                          '用户拥有此航旅标签',
                          '是否为实名认证用户',
                          '是否拥有用户勋章',
                          '是否拥有航旅可用卡券',
                          '是否是航旅APP新用户',
                          '判断是否是指定LBS位移id',
                          '动态传参(判断用户是否有可用卡券)',
                          'LBS位移',
                          '用户是否领取过某卡券',
                          '主站订单商品单价(单位为分)',
                          '主站订单消息类型',
                          '是否天猫超级会员',
                          '是否天猫APASS会员',
                          '用户是否在白名单中',
                          '是否拥有有效淘宝卡券(作废)',
                          '是否领取过淘宝卡券',
                          '是否属于人群',
                          '用户满送活动累计金额',
                          '交易订单是否包含本人',
                          '是否参与过问卷调查',
                          '是否属于春运5折类目黑名单',
                          '用户是否完成某个节点',
                          '是否使用过淘宝卡券',
                          '是否参与过问卷调查新版',
                          '支付订单含尾款支付',
                          '淘金币用户判断',
                          '是否预售订单',
                          '酒店checkOut时间',
                          '在酒店hid名单',
                          '在酒店卖家id名单',
                          'checkIn时间在此之后',
                          'checkOut时间在此之前',
                          'checkIn时间在此之前',
                          '订单创建在此之前',
                          '订单创建在此之后',
                          '是否酒店checkIn事件',
                          '是否信用住结账成功事件',
                          '是否酒店checkout事件',
                          '是否码上游支付',
                          '尾款支付在此之前',
                          '尾款支付在此之后',
                          '是否在booking特定hid名单A',
                          '是否在booking特定hid名单B',
                          '用户虚拟积分',
                          '伙伴会员注册状态',
                          '里程翻倍订单在此之前',
                          '里程翻倍订单在此之后',
                          '里程翻倍订单来源是否飞猪APP',
                          '用户是否有足够积分',
                          '用户互动虚拟积分',
                          '流水号查询用户互动虚拟积分',
                          '是否领取过指定活动中指定奖品组的奖品',
                          '用户互动虚拟积分不累计本次',
                          '是否领取过飞猪卡(T领取过F未领取)',
                          '永真规则',
                          '是否领取过淘宝Ump优惠券(new)',
                          '用户是否在白名单中(new)',
                          '用户是否有有效飞猪新卡卷(new)',
                          '用户是否参加过新玩法活动',
                          '判断星巴克活动人群',
                          '星巴克注册后发奖人群判断',
                          '是否在booking特定hid名单C',
                          '是否在booking特定hid名单I',
                          '是否在booking特定hid名单D',
                          '是否在booking特定hid名单E',
                          '是否在booking特定hid名单F',
                          '是否在booking特定hid名单G',
                          '是否在booking特定hid名单H',
                          '支付时是否持有有效的凭证券',
                          '用户是否领取过飞猪新卡卷-new',
                          '商品id白名单',
                          '批量查询用户是否拥有飞猪新卡券',
                          '订单号白名单',
                          '99梦想券抽奖资格判断',
                          '商品itemTag白名单',
                          '是否酒店日历房面付订单',
                          '是否是火车票抢票订单',
                          '订单商家实收款判断',
                          '用户有效的trippe积分判断',
                          '是否是预售订单',
                          '身份证在模版下未领取或已过期',
                          '订单支付状态判断',
                          '是否是飞猪app新下载用户',
                          '用户是否有有效飞猪日常卡',
                          '是否属于人群(new)',
                          '是否属于人群(不要用)',
                          '飞猪会员等级-升级',
                        ],
                      },
                      '3002_comparator': {
                        enumNames: [
                          '等于(数值)',
                          '大于（数值）',
                          '大于等于(数值)',
                          '小于(数值)',
                          '小于等于(数值)',
                        ],
                        enum: ['1', '9', '10', '12', '14'],
                      },
                      '3002_return_value': { type: 'string' },
                      '3004_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '3004_return_value': { type: 'string' },
                      '3008_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '3008_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '3008_params_tag': { type: 'string', title: '航旅标签' },
                      '4001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '4001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '5001_comparator': {
                        enumNames: [
                          '不相交(左右均集合)',
                          '包含(左集合包含右集合)',
                        ],
                        enum: ['17', '3001'],
                      },
                      '5001_return_value': {
                        enumNames: [
                          '哇哦饭Lv3',
                          '哇哦饭Lv4',
                          '哇哦饭Lv1',
                          '哇哦饭Lv2',
                          '活动积极分子Lv2',
                          '功盖神州',
                          '点评达人Lv2',
                          '点评达人Lv1',
                          '欧莱雅会员勋章',
                          '点评达人Lv3',
                          '海信电视会员勋章',
                          '维康会员勋章',
                          'yearonyear会员勋章',
                          '淘宝天下V5',
                          '华为会员勋章',
                          '哇哦饭Lv5',
                          '海尔会员勋章',
                          '哇哦饭Lv6',
                          '雅安加油',
                          '361°会员勋章',
                          '爱国者会员勋章',
                          'TCL会员勋章',
                          '海鸥会员勋章',
                          '神豆会员勋章',
                          '品牌会员勋章',
                          '资深买家Lv1',
                          '资深买家Lv2',
                          '乐扣乐扣会员勋章',
                          '合买达人',
                          '好奇会员勋章',
                          'ThinkPad会员勋章',
                          '长生鸟会员勋章',
                          '奥运英雄',
                          '多美滋会员勋章',
                          '美的会员勋章',
                          '中兴会员勋章',
                          '亚都会员勋章',
                          '一朵会员勋章',
                          '小有所成',
                          '实名认证',
                          '图享家Lv2',
                          '图享家Lv3',
                          '图享家Lv1',
                          '技嘉会员勋章',
                          '波斯顿会员勋章',
                          '圣元会员勋章',
                          '圣荷会员勋章',
                          '御泥坊会员勋章',
                          '活动积极分子Lv3',
                          'ABC会员勋章',
                          '公益使者',
                          '南极人会员勋章',
                          '诺基亚会员勋章',
                          '图享家Lv4',
                          '淘宝十周年',
                          '丽贝乐会员勋章',
                          '图享家Lv5',
                          '专辑潮人Lv1',
                          '丝蓓绮会员勋章',
                          'Bear小熊会员勋章',
                          '台电会员勋章',
                          '雀氏会员勋章',
                          'smart合伙人勋章',
                          '包邮卡',
                          '社交DJ  LV3',
                          '社交DJ  LV2',
                          '超级队长',
                          '怡丽会员勋章',
                          '御泥坊会员勋章',
                          '专辑潮人Lv3',
                          '迈途会员勋章',
                          '专辑潮人Lv2',
                          '社交DJ  LV1',
                          '创维会员勋章',
                          '奔腾电工会员勋章',
                          '联想手机会员勋章',
                          '特邀评论员Lv3',
                          '特邀评论员Lv4',
                          '买一善一',
                          '明一会员勋章',
                          '高丝会员勋章',
                          '特邀评论员Lv1',
                          '特邀评论员Lv2',
                          '好友爱',
                          '分享控Lv5',
                          '分享控Lv3',
                          '分享控Lv4',
                          '分享控Lv1',
                          '分享控Lv2',
                          '英伟达会员勋章',
                          '水星家纺会员勋章',
                          '游戏认证',
                          '施华蔻会员勋章',
                          '联想会员勋章',
                          '奥朵会员勋章',
                          '戴尔会员勋章',
                          '认证车主',
                          '笛莎会员勋章',
                          '笛莎会员勋章',
                          '金账户',
                          '帅康会员勋章',
                          '奔腾电器会员勋章',
                          '活动积极分子Lv1',
                          '昂达会员勋章',
                          '网购达人Lv1',
                          '网购达人Lv2',
                          '网购达人Lv3',
                          '亲情宝宝会员勋章',
                          '超级买家Lv1',
                          '超级买家Lv2',
                          '铁血君品行会员勋章',
                          '苏泊尔会员勋章',
                          '醉美四川',
                          '李宁会员勋章',
                          '飞利浦会员勋章',
                          'Hauswirt会员勋章',
                          '恋上party LV2',
                          '帮宝适会员勋章',
                          '恋上party LV1',
                          '试用先锋',
                          '马拉丁会员勋章',
                          '迪士尼会员勋章',
                          '凡茜会员勋章',
                          '领袖范Lv4',
                          '哇哦金卡',
                          '领袖范Lv5',
                          '领袖范Lv2',
                          '领袖范Lv3',
                          '领袖范Lv1',
                          '全球购会员',
                          '蜜丝佛陀会员勋章',
                          '相宜本草会员勋章',
                          '英特尔会员勋章',
                          '骆驼会员勋章',
                          '淘宝天下V6',
                          '美即会员勋章',
                          '亦美珊会员勋章',
                          '邦怡会员勋章',
                          '探路者会员勋章',
                          '雀巢会员勋章',
                          '宝家丽会员勋章',
                          '神舟会员勋章',
                          '珀莱雅会员勋章',
                          '电子券先锋',
                          '联合利华会员勋章',
                          '科沃斯会员勋章',
                          '母婴达人Lv1',
                          '富安娜会员勋章',
                          '华硕会员勋章',
                          '小狗会员勋章',
                          '母婴达人Lv3',
                          '惠氏会员勋章',
                          '母婴达人Lv2',
                          '宝洁会员勋章',
                          '判案先锋',
                          '判案达人',
                          '北极绒会员勋章',
                          '判案新秀',
                          '淘宝新人',
                          '淘宝信使',
                        ],
                        enum: [
                          '110303',
                          '110304',
                          '110301',
                          '110302',
                          '200901',
                          '102001',
                          '200502',
                          '200501',
                          '209201',
                          '200503',
                          '207301',
                          '202801',
                          '209601',
                          '200101',
                          '204701',
                          '110305',
                          '207701',
                          '110306',
                          '202401',
                          '205401',
                          '208901',
                          '203101',
                          '205001',
                          '210301',
                          '210701',
                          '101701',
                          '101702',
                          '208001',
                          '201601',
                          '206101',
                          '208401',
                          '203901',
                          '201201',
                          '205801',
                          '206501',
                          '208801',
                          '203501',
                          '204201',
                          '100101',
                          '100501',
                          '110402',
                          '110403',
                          '110401',
                          '209101',
                          '209103',
                          '209102',
                          '209501',
                          '207201',
                          '200801',
                          '202701',
                          '200401',
                          '209901',
                          '207601',
                          '110404',
                          '202301',
                          '204601',
                          '110405',
                          '201101',
                          '206901',
                          '203001',
                          '205301',
                          '210201',
                          '210601',
                          '300101',
                          '201503',
                          '201502',
                          '201901',
                          '208301',
                          '203801',
                          '201103',
                          '206401',
                          '201102',
                          '201501',
                          '205701',
                          '203401',
                          '208701',
                          '100603',
                          '100604',
                          '202201',
                          '204101',
                          '206001',
                          '100601',
                          '100602',
                          '110501',
                          '110105',
                          '110103',
                          '110104',
                          '110101',
                          '110102',
                          '209001',
                          '207101',
                          '200701',
                          '209401',
                          '204901',
                          '207501',
                          '202601',
                          '200301',
                          '209801',
                          '204501',
                          '101901',
                          '206801',
                          '203301',
                          '201001',
                          '205201',
                          '101101',
                          '101102',
                          '101103',
                          '210101',
                          '101501',
                          '101502',
                          '210501',
                          '208201',
                          '201801',
                          '206301',
                          '208601',
                          '203701',
                          '201402',
                          '205601',
                          '201401',
                          '202101',
                          '204401',
                          '207901',
                          '204001',
                          '110204',
                          '110601',
                          '110205',
                          '110202',
                          '110203',
                          '110201',
                          '200601',
                          '209301',
                          '207001',
                          '202901',
                          '207401',
                          '200201',
                          '202501',
                          '204801',
                          '209701',
                          '205501',
                          '206701',
                          '203201',
                          '205101',
                          '210401',
                          '201701',
                          '208101',
                          '206201',
                          '201301',
                          '205901',
                          '208501',
                          '203601',
                          '201303',
                          '206601',
                          '201302',
                          '207801',
                          '202002',
                          '202003',
                          '204301',
                          '202001',
                          '100001',
                          '100401',
                        ],
                      },
                      '9001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '9001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '9001_params_businessLine': {
                        type: 'string',
                        title: '类目',
                      },
                      '9001_params_templateId': {
                        type: 'string',
                        title: '模板Id',
                      },
                      '10001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '10001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '20002_comparator': {
                        enumNames: ['等于(数值)', '大于（数值）', '小于(数值)'],
                        enum: ['1', '9', '12'],
                      },
                      '20002_return_value': { type: 'string' },
                      '24001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '24001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '24004_comparator': {
                        enumNames: ['等于(数值)', '大于（数值）', '小于(数值)'],
                        enum: ['1', '9', '12'],
                      },
                      '24004_return_value': {
                        enumNames: [
                          '探亲位移',
                          '泰国',
                          '阿联酋',
                          '菲律宾',
                          '越南',
                          '印度尼西亚',
                          '广州',
                          '欧盟',
                          '韩国',
                          '马来西亚',
                          '苏州',
                          '英国',
                          '旅游位移',
                          '澳大利亚',
                          '上海',
                          '台湾',
                          '北京',
                          '美国',
                          '新加坡位移',
                          '港澳',
                          '日本',
                          '出差位移',
                          '南京',
                        ],
                        enum: [
                          '909',
                          '1022',
                          '1033',
                          '1032',
                          '1031',
                          '1030',
                          '1756',
                          '1018',
                          '980',
                          '1029',
                          '1755',
                          '1028',
                          '784',
                          '1027',
                          '1753',
                          '1026',
                          '1752',
                          '1025',
                          '963',
                          '1024',
                          '1023',
                          '910',
                          '1757',
                        ],
                      },
                      '26001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '26001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '26001_params_channel': {
                        type: 'string',
                        title: '渠道(非必填)',
                      },
                      '26001_params_templateId': {
                        type: 'string',
                        title: '卡券模版ID',
                      },
                      '27001_comparator': {
                        enumNames: [
                          '等于(数值)',
                          '大于（数值）',
                          '大于等于(数值)',
                          '小于(数值)',
                          '小于等于(数值)',
                        ],
                        enum: ['1', '9', '10', '12', '14'],
                      },
                      '27001_return_value': { type: 'string' },
                      '27002_comparator': {
                        enumNames: ['等于(字符)', '不等于(字符)'],
                        enum: ['2', '3'],
                      },
                      '27002_return_value': {
                        enumNames: ['预售支付订单'],
                        enum: ['10000-trade-paid-done'],
                      },
                      '35001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '35001_return_value': { type: 'string' },
                      '35002_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '35002_return_value': { type: 'string' },
                      '36001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '36001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '36001_params_userIdList': {
                        type: 'string',
                        title: 'userIdList',
                      },
                      '59001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '59001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '59001_params_umpInstanceIds': {
                        type: 'string',
                        title: 'ump模板Id',
                      },
                      '59002_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '59002_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '59002_params_umpInstanceIds': {
                        type: 'string',
                        title: 'ump模板Id',
                      },
                      '60001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '60001_return_value': { type: 'string' },
                      '60001_params_crowdId': {
                        type: 'string',
                        title: '人群ID',
                      },
                      '97001_comparator': {
                        enumNames: ['大于（数值）', '小于(数值)'],
                        enum: ['9', '12'],
                      },
                      '97001_return_value': { type: 'string' },
                      '115001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '115001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '116001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '116001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '116001_params_surveyId': {
                        type: 'string',
                        title: '问卷id',
                      },
                      '117001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '117001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '117001_params_categoryIdList': {
                        type: 'string',
                        title: 'categoryIdList',
                      },
                      '131001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '131001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '131001_params_nodeCode': {
                        type: 'string',
                        title: '节点ID',
                      },
                      '134001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '134001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '134001_params_umpInstanceIds': {
                        type: 'string',
                        title: 'ump模板Id',
                      },
                      '169001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '169001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '169001_params_surveyId': {
                        type: 'string',
                        title: '问卷id',
                      },
                      '170002_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '170002_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '173001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '173001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '173001_params_productId': {
                        type: 'string',
                        title: '产品id',
                      },
                      '173001_params_crowdId': {
                        type: 'string',
                        title: '人群id',
                      },
                      '173001_params_token': { type: 'string', title: '口令' },
                      '178001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '178001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '180001_comparator': {
                        enumNames: ['大于等于(数值)', '小于等于(数值)'],
                        enum: ['10', '14'],
                      },
                      '180001_return_value': { type: 'string' },
                      '181001_comparator': {
                        enumNames: ['等于(布尔)', '测试比较符管理'],
                        enum: ['6', '13001'],
                      },
                      '181001_return_value': { type: 'string' },
                      '181001_params_hidList': {
                        type: 'string',
                        title: '酒店hid列表',
                      },
                      '182001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '182001_return_value': { type: 'string' },
                      '182001_params_sellerIdList': {
                        type: 'string',
                        title: '卖家ID名单',
                      },
                      '182002_comparator': {
                        enumNames: ['等于(布尔)', '测试比较符管理'],
                        enum: ['6', '13001'],
                      },
                      '182002_return_value': { type: 'string' },
                      '182002_params_checkInDate2': {
                        type: 'string',
                        title: 'yyyy-MM-dd HH:mm:ss',
                      },
                      '183001_comparator': {
                        enumNames: ['等于(布尔)', '测试比较符管理'],
                        enum: ['6', '13001'],
                      },
                      '183001_return_value': { type: 'string' },
                      '183001_params_checkOutDate2': {
                        type: 'string',
                        title: 'yyyy-MM-dd HH:mm:ss',
                      },
                      '184001_comparator': {
                        enumNames: ['等于(布尔)', '测试比较符管理'],
                        enum: ['6', '13001'],
                      },
                      '184001_return_value': { type: 'string' },
                      '184001_params_checkInDate2': {
                        type: 'string',
                        title: 'yyyy-MM-dd HH:mm:ss',
                      },
                      '185001_comparator': {
                        enumNames: ['等于(布尔)', '测试比较符管理'],
                        enum: ['6', '13001'],
                      },
                      '185001_return_value': { type: 'string' },
                      '185001_params_orderCreateDate2': {
                        type: 'string',
                        title: 'yyyy-MM-dd HH:mm:ss',
                      },
                      '186001_comparator': {
                        enumNames: ['等于(布尔)', '测试比较符管理'],
                        enum: ['6', '13001'],
                      },
                      '186001_return_value': { type: 'string' },
                      '186001_params_orderCreateDate2': {
                        type: 'string',
                        title: 'yyyy-MM-dd HH:mm:ss',
                      },
                      '187001_comparator': {
                        enumNames: ['等于(布尔)', '测试比较符管理'],
                        enum: ['6', '13001'],
                      },
                      '187001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '188001_comparator': {
                        enumNames: ['等于(布尔)', '测试比较符管理'],
                        enum: ['6', '13001'],
                      },
                      '188001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '189001_comparator': {
                        enumNames: ['等于(布尔)', '测试比较符管理'],
                        enum: ['6', '13001'],
                      },
                      '189001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '194001_comparator': {
                        enumNames: ['等于(布尔)', '测试比较符管理'],
                        enum: ['6', '13001'],
                      },
                      '194001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '200001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '200001_return_value': { type: 'string' },
                      '200001_params_tailPaidDate3': {
                        type: 'string',
                        title: 'yyyy-MM-dd HH:mm:ss',
                      },
                      '201001_comparator': {
                        enumNames: ['等于(布尔)', '测试比较符管理'],
                        enum: ['6', '13001'],
                      },
                      '201001_return_value': { type: 'string' },
                      '201001_params_tailPaidDate2': {
                        type: 'string',
                        title: 'yyyy-MM-dd HH:mm:ss',
                      },
                      '207001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '207001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '208001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '208001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '212001_comparator': {
                        enumNames: [
                          '等于(数值)',
                          '大于（数值）',
                          '大于等于(数值)',
                          '小于(数值)',
                          '小于等于(数值)',
                          '不等于（数值）',
                        ],
                        enum: ['1', '9', '10', '12', '14', '12001'],
                      },
                      '212001_return_value': { type: 'string' },
                      '212001_params_virtralCode': {
                        type: 'string',
                        title: '虚拟积分ID',
                      },
                      '213001_comparator': {
                        enumNames: [
                          '等于(数值)',
                          '大于（数值）',
                          '大于等于(数值)',
                          '小于(数值)',
                          '小于等于(数值)',
                          '不等于（数值）',
                        ],
                        enum: ['1', '9', '10', '12', '14', '12001'],
                      },
                      '213001_return_value': { type: 'string' },
                      '213001_params_vendor': {
                        type: 'string',
                        title: '供应商',
                      },
                      '213001_params_partnerId': {
                        type: 'string',
                        title: '伙伴ID',
                      },
                      '215001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '215001_return_value': { type: 'string' },
                      '215001_params_payTime4PointDouble2': {
                        type: 'string',
                        title: 'yyyy-MM-dd HH:mm:ss',
                      },
                      '216001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '216001_return_value': { type: 'string' },
                      '216001_params_payTime4PointDouble3': {
                        type: 'string',
                        title: 'yyyy-MM-dd HH:mm:ss',
                      },
                      '219001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '219001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '220001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '220001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '221001_comparator': {
                        enumNames: [
                          '等于(数值)',
                          '大于（数值）',
                          '大于等于(数值)',
                          '小于(数值)',
                        ],
                        enum: ['1', '9', '10', '12'],
                      },
                      '221001_return_value': { type: 'string' },
                      '221001_params_virtualCode': {
                        type: 'string',
                        title: '虚拟积分id',
                      },
                      '222001_comparator': {
                        enumNames: [
                          '等于(数值)',
                          '大于（数值）',
                          '大于等于(数值)',
                          '小于(数值)',
                          '小于等于(数值)',
                        ],
                        enum: ['1', '9', '10', '12', '14'],
                      },
                      '222001_return_value': { type: 'string' },
                      '222001_params_virtualCode': {
                        type: 'string',
                        title: '虚拟积分id',
                      },
                      '223001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '223001_return_value': { type: 'string' },
                      '223001_params_subActId': {
                        type: 'string',
                        title: '活动id',
                      },
                      '223001_params_groupResourceId': {
                        type: 'string',
                        title: '资源组id',
                      },
                      '224001_comparator': {
                        enumNames: [
                          '等于(数值)',
                          '大于（数值）',
                          '大于等于(数值)',
                          '小于(数值)',
                          '小于等于(数值)',
                          '不等于（数值）',
                        ],
                        enum: ['1', '9', '10', '12', '14', '12001'],
                      },
                      '224001_return_value': { type: 'string' },
                      '224001_params_virtualCode': {
                        type: 'string',
                        title: '虚拟积分id',
                      },
                      '224001_params_virtualValue': {
                        type: 'string',
                        title: '本次新增积分',
                      },
                      '225001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '225001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '225001_params_subActId': {
                        type: 'string',
                        title: 'subActId',
                      },
                      '226001_comparator': {
                        enumNames: ['等于(布尔)', '测试比较符管理'],
                        enum: ['6', '13001'],
                      },
                      '226001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '229001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '229001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '229001_params_sellerId': {
                        type: 'string',
                        title: 'sellerId',
                      },
                      '229001_params_templateCode': {
                        type: 'string',
                        title: 'templateCode',
                      },
                      '230002_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '230002_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '230002_params_userIdList': {
                        type: 'string',
                        title: 'userIdList',
                      },
                      '231001_comparator': {
                        enumNames: ['等于(布尔)', '测试比较符管理'],
                        enum: ['6', '13001'],
                      },
                      '231001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '231001_params_templateId': {
                        type: 'string',
                        title: 'ump模板ID',
                      },
                      '232001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '232001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '232001_params_subActIdList': {
                        type: 'string',
                        title: '活动id(英文逗号隔开)',
                      },
                      '234001_comparator': {
                        enumNames: [
                          '等于(数值)',
                          '大于（数值）',
                          '大于等于(数值)',
                          '小于(数值)',
                          '小于等于(数值)',
                        ],
                        enum: ['1', '9', '10', '12', '14'],
                      },
                      '234001_return_value': { type: 'string' },
                      '234002_comparator': {
                        enumNames: [
                          '等于(数值)',
                          '大于（数值）',
                          '大于等于(数值)',
                          '小于(数值)',
                          '小于等于(数值)',
                          '不等于（数值）',
                        ],
                        enum: ['1', '9', '10', '12', '14', '12001'],
                      },
                      '234002_return_value': { type: 'string' },
                      '235001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '235001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '235002_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '235002_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '236001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '236001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '237001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '237001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '238001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '238001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '239001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '239001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '240001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '240001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '241001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '241001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '241001_params_subActTemplatedId': {
                        type: 'string',
                        title: '模版Id',
                      },
                      '244001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '244001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '244001_params_templateId': {
                        type: 'string',
                        title: 'ump模板ID',
                      },
                      '245001_comparator': {
                        enumNames: ['等于(布尔)', '测试比较符管理'],
                        enum: ['6', '13001'],
                      },
                      '245001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '245001_params_whiteItemIdList': {
                        type: 'string',
                        title: '商品Id白名单列表（多个用逗号分割）',
                      },
                      '250001_comparator': {
                        enumNames: ['等于(数值)'],
                        enum: ['1'],
                      },
                      '250001_return_value': {
                        enumNames: ['没有', '有'],
                        enum: ['0', '1'],
                      },
                      '250001_params_templateIds': {
                        type: 'string',
                        title: 'templateIds',
                      },
                      '251001_comparator': {
                        enumNames: ['等于(布尔)', '测试比较符管理'],
                        enum: ['6', '13001'],
                      },
                      '251001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '251001_params_tpOrderList': {
                        type: 'string',
                        title: '订单号list',
                      },
                      '251002_comparator': {
                        enumNames: ['等于(布尔)', '测试比较符管理'],
                        enum: ['6', '13001'],
                      },
                      '251002_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '251002_params_whiteOrderPayTime': {
                        type: 'string',
                        title: '支付完成限制时间',
                      },
                      '251002_params_whiteCategoryList': {
                        type: 'string',
                        title: '叶子类目白名单',
                      },
                      '252001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '252001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '252001_params_whiteItemTags': {
                        type: 'string',
                        title: '商品上的itemTag白名单（多个用逗号分割）',
                      },
                      '253001_comparator': {
                        enumNames: ['等于(数值)'],
                        enum: ['1'],
                      },
                      '253001_return_value': {
                        enumNames: ['否', '是'],
                        enum: ['0', '1'],
                      },
                      '255001_comparator': {
                        enumNames: ['等于(布尔)', '测试比较符管理'],
                        enum: ['6', '13001'],
                      },
                      '255001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '257001_comparator': {
                        enumNames: [
                          '等于(数值)',
                          '大于（数值）',
                          '大于等于(数值)',
                          '小于(数值)',
                          '小于等于(数值)',
                          '不等于（数值）',
                        ],
                        enum: ['1', '9', '10', '12', '14', '12001'],
                      },
                      '257001_return_value': { type: 'string' },
                      '258001_comparator': {
                        enumNames: [
                          '等于(数值)',
                          '大于（数值）',
                          '大于等于(数值)',
                          '小于(数值)',
                          '小于等于(数值)',
                          '不等于（数值）',
                        ],
                        enum: ['1', '9', '10', '12', '14', '12001'],
                      },
                      '258001_return_value': { type: 'string' },
                      '258001_params_bizType': {
                        type: 'string',
                        title: 'bizType',
                      },
                      '259001_comparator': {
                        enumNames: ['等于(布尔)', '测试比较符管理'],
                        enum: ['6', '13001'],
                      },
                      '259001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '261001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '261001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '261001_params_templateIds': {
                        type: 'string',
                        title: '券模版Id（多个用逗号分割）',
                      },
                      '271001_comparator': {
                        enumNames: ['等于(数值)'],
                        enum: ['1'],
                      },
                      '271001_return_value': { type: 'string' },
                      '273001_comparator': {
                        enumNames: ['等于(数值)'],
                        enum: ['1'],
                      },
                      '273001_return_value': {
                        enumNames: ['否', '是'],
                        enum: ['0', '1'],
                      },
                      '281001_comparator': {
                        enumNames: ['等于(布尔)'],
                        enum: ['6'],
                      },
                      '281001_return_value': {
                        enumNames: ['是', '否'],
                        enum: ['true', 'false'],
                      },
                      '281001_params_templateId': {
                        type: 'string',
                        title: '飞猪日常卡模板',
                      },
                      '285001_comparator': {
                        enumNames: ['等于(数值)'],
                        enum: ['1'],
                      },
                      '285001_return_value': {
                        enumNames: ['否', '是'],
                        enum: ['0', '1'],
                      },
                      '285001_params_crowdIds': {
                        type: 'string',
                        title: 'crowdIds',
                      },
                      '286001_comparator': {
                        enumNames: [
                          '等于(数值)',
                          '大于（数值）',
                          '大于等于(数值)',
                          '小于(数值)',
                          '小于等于(数值)',
                          '不等于（数值）',
                        ],
                        enum: ['1', '9', '10', '12', '14', '12001'],
                      },
                      '286001_return_value': { type: 'string' },
                      '286001_params_crowdIds': {
                        type: 'string',
                        title: 'crowdIds',
                      },
                      '289001_comparator': {
                        enumNames: [
                          '等于(数值)',
                          '大于（数值）',
                          '大于等于(数值)',
                          '小于(数值)',
                          '小于等于(数值)',
                          '不等于（数值）',
                        ],
                        enum: ['1', '9', '10', '12', '14', '12001'],
                      },
                      '289001_return_value': {
                        enumNames: [
                          'F0',
                          'F1(原F1)',
                          'F2(新F1+)',
                          'F3(原F2)',
                          'F4(原F3)',
                        ],
                        enum: ['0', '1', '2', '3', '4'],
                      },
                    },
                    type: 'object',
                    required: [],
                    name: 'ruleCondition',
                  },
                },
                actionId: {
                  title: '动作',
                  description: '',
                  type: 'string',
                  enum:
                    "{{formData.prizeGroupConfigList.prizeGroupList.map(obj => obj && obj.groupId ? obj.groupId.toString() : '$New' + obj.name)}}",
                  enumNames:
                    '{{formData.prizeGroupConfigList.prizeGroupList.map(obj => obj.name)}}',
                },
              },
              type: 'object',
              required: ['ruleDesc', 'unMatchNotice', 'actionId'],
              name: 'prizeSendRuleCfg',
            },
          },
        },
        type: 'object',
        required: [],
        title: '动态规则列表',
        name: 'prizeSendRuleList',
      },
      applyGiftPackageShowConfig: {
        properties: {
          template: {
            title: '模板',
            description: '',
            type: 'string',
            enum: ['1', '2', '3', '4'],
            enumNames: [
              '非会场通用模板',
              '会场模板',
              '会场素材模板(弹窗表达)',
              '支付宝消费券及推荐模板(券库同步专用，慎选)',
            ],
            default: '1',
          },
          giftPkgName: { title: '奖品名称', description: '', type: 'string' },
          giftPkgDescription: {
            title: '奖品描述',
            description: '',
            type: 'string',
          },
          faceValue: {
            title: '面额',
            description: '',
            type: 'string',
            enum: ['1', '2', '3', '4', '5'],
            enumNames: [
              '固定金额',
              '固定折扣',
              '减至',
              '纯文案',
              '自定义前中后缀',
            ],
          },
          fixedAmountBefore: {
            title: '',
            description: '',
            type: 'string',
            pattern: '^([1-9]\\d*|0)$',
          },
          fixedAmountAfter: {
            title: '',
            description: '',
            type: 'string',
            pattern: '^([1-9]\\d*)$',
          },
          discount: {
            title: '',
            description: '1-9，支持小数点后一位',
            type: 'string',
            pattern: '^((0\\.[1-9]{1})|(([1-9]{1})(\\.\\d{1})?))$',
          },
          reduce: {
            title: '',
            description: '',
            type: 'string',
            pattern: '^([1-9]\\d*)$',
          },
          faceValuePurePrefixText: {
            title: '前缀',
            description: '',
            type: 'string',
          },
          faceValuePureInfixText: {
            title: '中缀',
            description: '',
            type: 'string',
          },
          faceValuePureText: {
            title:
              "{{rootValue.template=='2'?'后缀':rootValue.template=='1'?rootValue.faceValue=='5'?'后缀':'':rootValue.template=='3'?'后缀':''}}",
            description: '',
            type: 'string',
          },
          effectiveTimeType: {
            title: '使用有效期',
            description: '',
            type: 'string',
            enum: ['1', '2', '3', '4'],
            enumNames: ['固定时间', '自领取之时起', '自领取之日起', '纯文案'],
          },
          beginTime: { title: '开始时间', description: '', type: 'string' },
          endTime: { title: '结束时间', description: '', type: 'string' },
          effectiveMin: {
            title: '',
            description: '',
            type: 'string',
            pattern: '^[0-9]+$',
          },
          effectiveDay: {
            title: '',
            description: '',
            type: 'string',
            pattern: '^[0-9]+$',
          },
          effectTimePureText: { title: '', description: '', type: 'string' },
          notAvailableJumpurl: {
            title: '不可领引导跳转链接',
            description: '',
            type: 'string',
            pattern:
              '^(https?|ftp|file)://[-a-zA-Z0-9+&@#/%?=~_|!:,.;]*[-a-zA-Z0-9+&@#/%=~_|]',
          },
          outOfStockJumpUrl: {
            title: '活动无库存引导跳转链接',
            description: '',
            type: 'string',
            pattern:
              '^(https?|ftp|file)://[-a-zA-Z0-9+&@#/%?=~_|!:,.;]*[-a-zA-Z0-9+&@#/%=~_|]',
          },
          toUseUrl4Pc: {
            title: '奖品去使用链接(PC)',
            description: '',
            type: 'string',
          },
          toUseUrl4Wx: {
            title: '奖品去使用链接(无线)',
            description: '',
            type: 'string',
          },
          subscriptText: {
            title: '角标文案',
            description: '在会场上券模块左上角展示的文案',
            type: 'string',
            maxLength: 6,
          },
          doubleCheckPointText: {
            title: '二次确认框文案',
            description: '如：你确认花费x里程参与抽奖(兑换)吗？',
            type: 'string',
            maxLength: 20,
          },
          useRestrictionsText: {
            title: '使用限制文案',
            description: '如：满3000可用，仅F3会员可用',
            type: 'string',
            maxLength: 1024,
          },
          wonToastText: {
            title: '中奖弹窗按钮文案',
            description: '中奖弹窗按钮文案',
            type: 'string',
            maxLength: 20,
            default: '知道了',
          },
          wonToastBtnJumpUrl: {
            title: '中奖弹窗按钮跳转地址',
            description: '',
            type: 'string',
          },
          notWonToastText: {
            title: '未中奖弹窗按钮文案',
            description: '未中奖弹窗按钮文案',
            type: 'string',
            maxLength: 20,
            default: '再抽一次',
          },
          notWonToastBtnJumpUrl: {
            title: '未中奖弹窗按钮跳转地址',
            description: '未中奖弹窗按钮跳转地址',
            type: 'string',
          },
          couponRingType: {
            title: '优惠券圈品方式',
            description: '',
            type: 'string',
            enum: ['1'],
            enumNames: ['海神id'],
          },
          couponRingValue: {
            title: '圈品标志',
            description: '',
            type: 'string',
          },
          alipayCouponStockActChannels: {
            title: '活动渠道',
            description: '',
            type: 'array',
            items: { type: 'string' },
            enum: ['HOME_PAGE', 'NATIONAL_DAY_PROMOTION'],
            enumNames: ['首页SVU/SSU', '会场'],
          },
          alipayCouponStockBusinessLine: {
            title: '业务线',
            description: '',
            type: 'string',
            enum: [
              '通用',
              '国际机票',
              '国内机票',
              '火车票',
              '汽车票',
              '用车',
              '酒店',
              '度假',
              '门票',
            ],
            enumNames: [
              '通用',
              '国际机票',
              '国内机票',
              '火车票',
              '汽车票',
              '用车',
              '酒店',
              '度假',
              '门票',
            ],
          },
          alipayCouponStockDeliveryCityCode: {
            title: '投放城市（6位行政区划,多个英文逗号分隔）',
            description: '全国填 ALL ;6位行政区划,多个英文逗号分隔',
            type: 'string',
            pattern: '^(ALL)?([,]?(\\d){6})*$',
          },
          kvConfig: {
            title: '自定义素材',
            description: '',
            type: 'array',
            items: { type: 'string' },
            enum: [''],
          },
        },
        type: 'object',
        required: [
          'template',
          'giftPkgName',
          'faceValue',
          'fixedAmountBefore',
          'fixedAmountAfter',
          'discount',
          'reduce',
          'faceValuePureText',
          'effectiveTimeType',
          'beginTime',
          'endTime',
          'effectiveMin',
          'effectiveDay',
          'effectTimePureText',
          'alipayCouponStockBusinessLine',
        ],
        title: '用户端礼包展示信息',
        name: 'applyGiftPackageShowConfig',
      },
      applyButtonShowConfig: {
        properties: {
          showAdvancedSetting: {
            title: '高级配置',
            description: '',
            type: 'boolean',
          },
          activeNotReachedStartTime: {
            title: '未到活动开始时间',
            description: '',
            type: 'string',
            default: '未开始',
          },
          activeIsOver: {
            title: '活动已结束',
            description: '',
            type: 'string',
            default: '已结束',
          },
          notAvailable: {
            title: '不可领取',
            description: '',
            type: 'string',
            default: '不可领',
          },
          available: {
            title: '可参与',
            description: '',
            type: 'string',
            default: '立即抢',
          },
          received: {
            title: '已领取',
            description: '',
            type: 'string',
            default: '已领取',
          },
          outOfStock: {
            title: '库存不足（玩法或奖品）',
            description: '',
            type: 'string',
            default: '已抢完',
          },
          toUse: {
            title: '去使用',
            description: '',
            type: 'string',
            default: '去使用',
          },
          notWonAndNotAvailable: {
            title: '抽完未中',
            description: '',
            type: 'string',
            default: '已抽过',
          },
          userParticipateLimitReached: {
            title: '参与次数到上限',
            description: '',
            type: 'string',
            default: '不可领',
          },
        },
        type: 'object',
        required: [],
        title: '用户端按钮展示信息',
        name: 'applyButtonShowConfig',
      },
      applyPromptConfig: {
        properties: {
          showAdvancedSetting: {
            title: '高级配置',
            description: '',
            type: 'boolean',
          },
          applySuccessMainTitle: {
            title: '成功提示(主标题)',
            description: '会场领券后的弹框主标题',
            type: 'string',
            max: 6,
            default: '恭喜您',
          },
          applyFailedMainTitle: {
            title: '失败提示(主标题)',
            description: '会场领券后的弹框主标题',
            type: 'string',
            max: 6,
            default: '很遗憾',
          },
          applySuccess: {
            title: '参与成功',
            description: '',
            type: 'string',
            default: '恭喜您！',
          },
          lotteryNotWon: {
            title: '未中奖',
            description: '',
            type: 'string',
            default: '你和奖品只差一丢丢',
          },
          activeNotReachedStartTime: {
            title: '活动未到开始时间',
            description: '',
            type: 'string',
            default: '亲，活动尚未开始！',
          },
          activityHasExpired: {
            title: '已过活动结束时间',
            description: '',
            type: 'string',
            default: '亲，活动已结束！',
          },
          outOfStock: {
            title: '库存不足',
            description: '',
            type: 'string',
            default: '已被抢光啦，再看看别的好福利吧！',
          },
          userTotalParticipateLimitReached: {
            title: '单用户总参与次数到达上限',
            description: '',
            type: 'string',
            default: '参与次数已达上限',
          },
          userDailyParticipateLimitReached: {
            title: '单用户每天参与次数到达上限',
            description: '',
            type: 'string',
            default: '今日参与次数已达上限，明天再来试试～',
          },
          userWeeklyParticipateLimitReached: {
            title: '单用户每周参与次数到达上限',
            description: '',
            type: 'string',
            default: '本周参与次数已达上限，下周再来试试～',
          },
          userTotalLotteryWonLimitReached: {
            title: '单用户总中奖次数到达上限',
            description: '',
            type: 'string',
            default: '中奖次数已达上限',
          },
          userDailyLotteryWonLimitReached: {
            title: '单用户每天总中奖次数到达上限',
            description: '',
            type: 'string',
            default: '今日中奖次数已达上限，明天再来试试～',
          },
          userWeeklyLotteryWonLimitReached: {
            title: '单用户每周总中奖次数到达上限',
            description: '',
            type: 'string',
            default: '本周中奖次数已达上限，下周再来试试～',
          },
          userGroupLotteryWonLimitReached: {
            title: '单用户奖品组中奖次数到达上限',
            description: '',
            type: 'string',
            default: '本活动中奖次数已达上限',
          },
          securityBlock: {
            title: '安全码等身份验证不通过',
            description: '',
            type: 'string',
            default: '你和奖品只差一丢丢',
          },
          participationConditionsNotReached: {
            title: '触发参与条件不满足',
            description: '',
            type: 'string',
            default: '你和奖品只差一丢丢',
          },
          deductPropertyLacking: {
            title: '扣减资源不足',
            description:
              '{扣减资源名称}会自动替换为上面选择的资源类型请勿随意更改',
            type: 'string',
            default: '您的{扣减资源名称}不足，无法参与活动',
          },
          bottomPrompt: {
            title: '其他原因发奖失败',
            description: '',
            type: 'string',
            default: '你和奖品只差一丢丢',
          },
        },
        type: 'object',
        required: [],
        title: '用户端提示文案',
        name: 'applyPromptConfig',
      },
      generalExt: {
        properties: {
          kvConfig: {
            title: '扩展key-value',
            description: '',
            type: 'array',
            items: { type: 'string' },
            enum: [''],
          },
        },
        type: 'object',
        required: [],
        title: '扩展信息',
        name: 'generalExt',
      },
      wfCommonInfo: {
        name: 'wfCommonInfo',
        type: 'object',
        properties: {
          productCode: {},
          bizCode: {},
          theTwoEditors: {},
          subActStatus: {},
        },
      },
    },
    type: 'object',
  },
  uiSchema: {
    crowd: {
      upfIdType: { 'ui:width': '50%' },
      upfIdList: { 'ui:width': '50%' },
      dreamCouponLotteryCheck: {
        'ui:hidden':
          "@['LOTTERY','AWARD'].indexOf(formData.wfCommonInfo.productCode)===-1",
      },
    },
    limit: {
      totalLimitCount: { 'ui:width': '33%' },
      addAllCount: {
        'ui:hidden': "hasAddItems == '1'",
        'ui:disabled': true,
        'ui:width': '33%',
      },
      totalLimitId: { 'ui:hidden': true },
      surplusCount: { 'ui:disabled': true, 'ui:width': '33%' },
      noActiveAllCount: {
        'ui:hidden': "hasAddItems == '1'",
        'ui:disabled': true,
        'ui:width': '33%',
      },
      userLimitTotal: { 'ui:width': '33%' },
      showAdvancedSetting: {
        'ui:hidden':
          '{{formData.integralCertificateCouponUseValidity && formData.integralCertificateCouponUseValidity.sellCouponFlag===true}}',
      },
      userLimitDay: {
        'ui:hidden': 'showAdvancedSetting==false',
        'ui:width': '33%',
      },
      userLimitWeek: {
        'ui:hidden': 'showAdvancedSetting==false',
        'ui:width': '33%',
      },
      userLimitNaturalMonth: {
        'ui:hidden': 'showAdvancedSetting==false',
        'ui:width': '33%',
      },
      limitDay: {
        'ui:hidden': 'showAdvancedSetting==false',
        'ui:width': '33%',
      },
      limitWeek: {
        'ui:hidden': 'showAdvancedSetting==false',
        'ui:width': '33%',
      },
      limitNaturalMonth: {
        'ui:hidden': 'showAdvancedSetting==false',
        'ui:width': '33%',
      },
      userWinningLimitTotal: { 'ui:hidden': true, 'ui:width': '33%' },
      userWinningLimitDay: { 'ui:hidden': true, 'ui:width': '33%' },
      userWinningLimitWeek: { 'ui:hidden': true, 'ui:width': '33%' },
      latour2SendFliggyPackage: { 'ui:hidden': true, 'ui:width': '50%' },
      hasAddItems: { 'ui:hidden': true },
      groupAddLimitText: {
        'ui:widget': 'text',
        'ui:hidden': 'showAdvancedSetting == false',
      },
      incrInventoryItems: {
        'ui:options': { foldable: true },
        'ui:hidden': 'showAdvancedSetting==false',
        items: {
          addTime: {
            'ui:widget': 'date',
            'ui:disabled': false,
            'ui:width': '40%',
          },
          addCount: { 'ui:disabled': false, 'ui:width': '30%' },
          validCount: { 'ui:disabled': true, 'ui:width': '30%' },
          taskId: { 'ui:hidden': true },
          itemId: { 'ui:hidden': true },
        },
      },
    },
    prizeGroupConfigList: {
      prizeGroupList: {
        'ui:options': { foldable: true },
        items: {
          lotteryProbability: { 'ui:widget': 'slider', 'ui:hidden': true },
          groupId: { 'ui:hidden': true },
          groupIdVO: { 'ui:disabled': true },
          inventoryId: { 'ui:hidden': true },
          inventoryLimit: { 'ui:width': '50%' },
          groupAddAllCount: {
            'ui:hidden': "groupHasAddItems == '1'",
            'ui:disabled': true,
            'ui:width': '33%',
          },
          surplusCount: { 'ui:disabled': true, 'ui:width': '50%' },
          groupNoActiveAllCount: {
            'ui:hidden': "groupHasAddItems == '1'",
            'ui:disabled': true,
            'ui:width': '33%',
          },
          userGroupWinningLimitTotal: {
            'ui:hidden':
              "@['LOTTERY'].indexOf(formData.wfCommonInfo.productCode)===-1",
            'ui:width': '50%',
          },
          userGroupWinningLimitDay: {
            'ui:hidden':
              "@['LOTTERY'].indexOf(formData.wfCommonInfo.productCode)===-1 || groupShowAdvancedSetting == false",
            'ui:width': '50%',
          },
          userGroupWinningLimitWeek: {
            'ui:hidden':
              "@['LOTTERY'].indexOf(formData.wfCommonInfo.productCode)===-1 || groupShowAdvancedSetting == false",
            'ui:width': '50%',
          },
          lotteryApplyMsg: {
            'ui:options': { placeholder: '如：恭喜您抽中xx奖品' },
            'ui:hidden':
              "@['LOTTERY'].indexOf(formData.wfCommonInfo.productCode)===-1",
          },
          groupAddLimitText: {
            'ui:widget': 'text',
            'ui:hidden': 'groupShowAdvancedSetting == false',
          },
          groupHasAddItems: { 'ui:hidden': true },
          groupIncrInventoryItems: {
            'ui:options': { foldable: true },
            'ui:hidden': 'groupShowAdvancedSetting == false',
            items: {
              groupAddTime: {
                'ui:widget': 'date',
                'ui:disabled': false,
                'ui:width': '40%',
              },
              groupAddCount: { 'ui:disabled': false, 'ui:width': '30%' },
              groupValidCount: { 'ui:disabled': true, 'ui:width': '30%' },
              groupTaskId: { 'ui:hidden': true },
              groupItemId: { 'ui:hidden': true },
            },
          },
          prizeConfig: { 'ui:widget': 'text' },
          prizeList: {
            'ui:options': { foldable: true },
            items: {
              fliggyNewCouponType: { 'ui:hidden': true },
              rightsId: {
                'ui:hidden':
                  "prizeType=='FLIGGY_F2_VIP' || prizeType=='PUSH_MESSAGE' || prizeType=='FLIGGY_MILEAGE' || prizeType=='ALITRIP_EXPERIENCE' || prizeType == 'THURSDAY_DOUBLE_MILES' || prizeType=='COMPLETE_TASK_NODE' || prizeType=='SUPER_VIP_CUSTOM_MULTI_MILES'",
              },
              channel: {
                'ui:hidden':
                  "prizeType!='ALITRIP_UMP_COMMON_COUPON' && prizeType!='THURSDAY_DOUBLE_MILES' && prizeType!='SUPER_VIP_CUSTOM_MULTI_MILES' && prizeType!='LATOUR2_BENEFIT' && prizeType!='ALITRIP_EXPERIENCE' && prizeType!='FLIGGY_TCCP_RIGHT' && prizeType!='FLIGGY_RIGHT'",
              },
              source: {
                'ui:widget': 'asyncSelect2',
                'ui:hidden':
                  "prizeType!='FLIGGY_NEW_COUPON' && prizeType!='ALIPAY318_FLIGGY_NEW_COUPON' && prizeType!='CYCLE_FLIGGY_NEW_COUPON'",
              },
              pushBizCode: { 'ui:hidden': "prizeType!='PUSH_MESSAGE'" },
              actionCode: { 'ui:hidden': "prizeType!='PUSH_MESSAGE'" },
              pushExtInfo: { 'ui:hidden': "prizeType!='PUSH_MESSAGE'" },
              virtualValue: {
                'ui:hidden': "prizeType!='VIRTUAL_BONUS_POINTS'",
              },
              durationDays: {
                'ui:hidden':
                  "prizeType!='FLIGGY_F2_VIP' && prizeType!='FLIGGY_TCCP_RIGHT' && prizeType!='FLIGGY_RIGHT'",
              },
              vipActiveChannel: { 'ui:hidden': "prizeType!='FLIGGY_F1_VIP'" },
              businessLine: {
                'ui:hidden':
                  "prizeType!='FLIGGY_NEW_COUPON' && prizeType!='ALIPAY318_FLIGGY_NEW_COUPON' && prizeType!='CYCLE_FLIGGY_NEW_COUPON'",
              },
              couponDesc: {
                'ui:hidden':
                  "prizeType=='PUSH_MESSAGE' || prizeType=='WRITE_OFF_CERTIFICATE_COUPON' || prizeType=='CERTIFICATE_COUPON_PACKAGE' || prizeType =='THURSDAY_DOUBLE_MILES'|| prizeType =='SUPER_VIP_CUSTOM_MULTI_MILES' || prizeType=='COMPLETE_TASK_NODE'",
              },
              couponDescPic: {
                'ui:hidden':
                  "prizeType=='FLIGGY_CARD_PACKAGE'||prizeType=='PUSH_MESSAGE' || prizeType=='WRITE_OFF_CERTIFICATE_COUPON' || prizeType=='CERTIFICATE_COUPON_PACKAGE' || prizeType =='THURSDAY_DOUBLE_MILES'|| prizeType =='SUPER_VIP_CUSTOM_MULTI_MILES' || prizeType=='COMPLETE_TASK_NODE'",
              },
              couponUseUrl: {
                'ui:hidden':
                  "prizeType=='PUSH_MESSAGE'|| prizeType=='WRITE_OFF_CERTIFICATE_COUPON' || prizeType=='CERTIFICATE_COUPON_PACKAGE' || prizeType =='THURSDAY_DOUBLE_MILES'|| prizeType =='SUPER_VIP_CUSTOM_MULTI_MILES' || prizeType=='COMPLETE_TASK_NODE'",
              },
              hidden: { 'ui:hidden': "prizeType!='FLIGGY_NEW_COUPON'" },
              resourceId: { 'ui:hidden': true },
              rightsSellerType: { 'ui:hidden': "prizeType!='BENEFIT_CENTER'" },
              exchangeTotal: { 'ui:hidden': "prizeType!='FLIGGY_RIGHT'" },
              taobaoCouponInstanceSource: {
                'ui:hidden': "prizeType!='TAOBAO_COUPON'",
              },
              taobaoCouponQuerySource: {
                'ui:hidden': "prizeType!='TAOBAO_COUPON'",
              },
              taobaoCouponSpreadId: {
                'ui:hidden': "prizeType!='TAOBAO_COUPON'",
              },
              taobaoCouponSupplierId: {
                'ui:hidden': "prizeType!='TAOBAO_COUPON'",
              },
              taobaoCouponUUID: { 'ui:hidden': "prizeType!='TAOBAO_COUPON'" },
              fliggyNewCouponCycleType: {
                'ui:hidden':
                  "prizeType!='CYCLE_FLIGGY_NEW_COUPON' && prizeType!='FLIGGY_VOUCHER'",
              },
              mileageCount: { 'ui:hidden': "prizeType!='FLIGGY_MILEAGE'" },
              mileageMemo: { 'ui:hidden': "prizeType!='FLIGGY_MILEAGE'" },
              faceValue: {
                'ui:hidden':
                  "showAdvancedSetting==false || prizeType=='FLIGGY_F2_VIP'||prizeType=='VIRTUAL_BONUS_POINTS'",
              },
              fixedAmountBefore: {
                'ui:options': { addonBefore: '满', addonAfter: '元' },
                'ui:hidden':
                  "showAdvancedSetting==false ||faceValue!='1'||prizeType=='FLIGGY_F2_VIP'||prizeType=='VIRTUAL_BONUS_POINTS'",
                'ui:width': '50%',
              },
              fixedAmountAfter: {
                'ui:options': { addonBefore: '减', addonAfter: '元' },
                'ui:hidden':
                  "showAdvancedSetting==false ||faceValue!='1'||prizeType=='FLIGGY_F2_VIP'||prizeType=='VIRTUAL_BONUS_POINTS'",
                'ui:width': '50%',
              },
              discount: {
                'ui:options': { addonAfter: '折' },
                'ui:hidden':
                  "showAdvancedSetting==false ||faceValue!='2'||prizeType=='FLIGGY_F2_VIP'||prizeType=='VIRTUAL_BONUS_POINTS'",
                'ui:width': '50%',
              },
              reduce: {
                'ui:options': { addonAfter: '元' },
                'ui:hidden':
                  "showAdvancedSetting==false ||faceValue!='3'||prizeType=='FLIGGY_F2_VIP'||prizeType=='VIRTUAL_BONUS_POINTS'",
                'ui:width': '50%',
              },
              faceValuePurePrefixText: {
                'ui:hidden': "showAdvancedSetting==false || faceValue!='5'",
                'ui:width': '50%',
              },
              faceValuePureInfixText: {
                'ui:hidden': "showAdvancedSetting==false || faceValue!='5'",
                'ui:width': '50%',
              },
              faceValuePureText: {
                'ui:hidden':
                  "{{ !(rootValue.showAdvancedSetting==false && rootValue.faceValue=='4' ) || !( rootValue.showAdvancedSetting==false && rootValue.faceValue=='5' ) }}",
              },
              effectiveTimeType: {
                'ui:hidden':
                  "showAdvancedSetting==false ||prizeType=='FLIGGY_F2_VIP'||prizeType=='VIRTUAL_BONUS_POINTS'",
              },
              beginTime: {
                'ui:widget': 'date',
                'ui:hidden':
                  "showAdvancedSetting==false ||effectiveTimeType!='1'||prizeType=='FLIGGY_F2_VIP'||prizeType=='VIRTUAL_BONUS_POINTS'",
                'ui:width': '50%',
              },
              endTime: {
                'ui:widget': 'date',
                'ui:hidden':
                  "showAdvancedSetting==false ||effectiveTimeType!='1'||prizeType=='FLIGGY_F2_VIP'||prizeType=='VIRTUAL_BONUS_POINTS'",
                'ui:width': '50%',
              },
              effectiveMin: {
                'ui:options': { addonAfter: '分钟' },
                'ui:hidden':
                  "showAdvancedSetting==false ||effectiveTimeType!='2'",
                'ui:width': '50%',
              },
              effectiveDay: {
                'ui:options': { addonAfter: '天' },
                'ui:hidden':
                  "showAdvancedSetting==false ||effectiveTimeType!='3'",
                'ui:width': '50%',
              },
              effectTimePureText: {
                'ui:hidden':
                  "showAdvancedSetting==false ||effectiveTimeType!='4'",
              },
              toUseUrl4Pc: {
                'ui:hidden':
                  "showAdvancedSetting==false ||prizeType=='FLIGGY_F2_VIP'||prizeType=='VIRTUAL_BONUS_POINTS'|| prizeType=='WRITE_OFF_CERTIFICATE_COUPON'",
              },
              toUseUrl4Wx: {
                'ui:hidden':
                  "showAdvancedSetting==false ||prizeType=='FLIGGY_F2_VIP'||prizeType=='VIRTUAL_BONUS_POINTS'|| prizeType=='WRITE_OFF_CERTIFICATE_COUPON'",
              },
              tag: { 'ui:hidden': "prizeType!='ALITRIP_TAG'" },
              count: {
                'ui:hidden':
                  "prizeType!='MOVIE_COUPON' || prizeType!='FLIGGY_VOUCHER'",
              },
              desc: {
                'ui:hidden':
                  "prizeType!='MOVIE_COUPON' || prizeType!='ALIPAY_RED_PACKED' || prizeType!='ALIPAY_OFFLINE_VOUCHER'",
              },
              url: { 'ui:hidden': "prizeType!='MOVIE_COUPON'" },
              partnerCode: { 'ui:hidden': "prizeType!='MOVIE_COUPON'" },
              amount: {
                'ui:hidden':
                  "prizeType!='MOVIE_COUPON' && prizeType!='ALIPAY_RED_PACKED' && prizeType!='ALITRIP_EXPERIENCE' && prizeType!='FUND_PLATFORM_SEND' && prizeType!='ALITRIP_COUPON'",
              },
              cashPoolId: { 'ui:hidden': "prizeType!='FUND_PLATFORM_SEND'" },
              cashPoolUserId: {
                'ui:hidden': "prizeType!='FUND_PLATFORM_SEND'",
              },
              subBusinessType: {
                'ui:hidden': "prizeType!='FUND_PLATFORM_SEND'",
              },
              priceReduce: { 'ui:hidden': "prizeType!='FUND_PLATFORM_SEND'" },
              securityCode: { 'ui:hidden': "prizeType!='FUND_PLATFORM_SEND'" },
              topPrice: { 'ui:hidden': "prizeType!='FUND_PLATFORM_SEND'" },
              stgCode: { 'ui:hidden': "prizeType!='LATOUR2_BENEFIT'" },
              multiple: {
                'ui:hidden':
                  "prizeType!='THURSDAY_DOUBLE_MILES' && prizeType!='SUPER_VIP_CUSTOM_MULTI_MILES'",
              },
              maxSendMiles: {
                'ui:hidden':
                  "prizeType!='THURSDAY_DOUBLE_MILES' && prizeType!='SUPER_VIP_CUSTOM_MULTI_MILES'",
              },
              subActMaxSendMiles: {
                'ui:hidden':
                  "prizeType!='THURSDAY_DOUBLE_MILES' && prizeType!='SUPER_VIP_CUSTOM_MULTI_MILES'",
              },
              dreamCouponMaxAmount: {
                'ui:hidden': "prizeType!='DREAM_COUPON_VIRTUAL_BONUS_POINTS' ",
              },
              dreamCouponPrizeType: {
                'ui:hidden': "prizeType!='DREAM_COUPON_VIRTUAL_BONUS_POINTS'",
              },
              dreamCouponMatchVirtualScoreActivityId: {
                'ui:hidden':
                  "prizeType!='RANDOM_AMT_ALIPAY_RED_PACKED_4_DREAM_COUPON' && prizeType!='RANDOM_AMT_FLIGGY_RED_PACKED_4_DREAM_COUPON'",
              },
            },
          },
        },
      },
    },
    prizeSendRuleList: {
      prizeSendRuleList: {
        'ui:options': { foldable: true },
        items: {
          ruleDesc: { 'ui:width': '50%' },
          unMatchNotice: { 'ui:width': '50%' },
          strategyId: { 'ui:hidden': true },
          ruleConditionCfg: {
            items: {
              conditionName: {
                'ui:options': { showSearch: true },
                'ui:width': '45%',
              },
              '3002_comparator': {
                'ui:hidden': 'conditionName!=3002',
                'ui:width': '15%',
              },
              '3002_return_value': {
                'ui:hidden': 'conditionName!=3002',
                'ui:width': '40%',
              },
              '3004_comparator': {
                'ui:hidden': 'conditionName!=3004',
                'ui:width': '15%',
              },
              '3004_return_value': {
                'ui:hidden': 'conditionName!=3004',
                'ui:width': '40%',
              },
              '3008_params_tag': {
                'ui:hidden': 'conditionName!=3008',
                'ui:width': '40%',
              },
              '3008_comparator': {
                'ui:hidden': 'conditionName!=3008',
                'ui:width': '15%',
              },
              '3008_return_value': {
                'ui:hidden': 'conditionName!=3008',
                'ui:width': '40%',
              },
              '4001_comparator': {
                'ui:hidden': 'conditionName!=4001',
                'ui:width': '15%',
              },
              '4001_return_value': {
                'ui:hidden': 'conditionName!=4001',
                'ui:width': '40%',
              },
              '5001_comparator': {
                'ui:hidden': 'conditionName!=5001',
                'ui:width': '15%',
              },
              '5001_return_value': {
                'ui:hidden': 'conditionName!=5001',
                'ui:width': '40%',
              },
              '9001_params_businessLine': {
                'ui:hidden': 'conditionName!=9001',
                'ui:width': '40%',
              },
              '9001_params_templateId': {
                'ui:hidden': 'conditionName!=9001',
                'ui:width': '40%',
              },
              '9001_comparator': {
                'ui:hidden': 'conditionName!=9001',
                'ui:width': '15%',
              },
              '9001_return_value': {
                'ui:hidden': 'conditionName!=9001',
                'ui:width': '40%',
              },
              '10001_comparator': {
                'ui:hidden': 'conditionName!=10001',
                'ui:width': '15%',
              },
              '10001_return_value': {
                'ui:hidden': 'conditionName!=10001',
                'ui:width': '40%',
              },
              '20002_comparator': {
                'ui:hidden': 'conditionName!=20002',
                'ui:width': '15%',
              },
              '20002_return_value': {
                'ui:hidden': 'conditionName!=20002',
                'ui:width': '40%',
              },
              '24001_comparator': {
                'ui:hidden': 'conditionName!=24001',
                'ui:width': '15%',
              },
              '24001_return_value': {
                'ui:hidden': 'conditionName!=24001',
                'ui:width': '40%',
              },
              '24004_comparator': {
                'ui:hidden': 'conditionName!=24004',
                'ui:width': '15%',
              },
              '24004_return_value': {
                'ui:hidden': 'conditionName!=24004',
                'ui:width': '40%',
              },
              '26001_params_channel': {
                'ui:hidden': 'conditionName!=26001',
                'ui:width': '40%',
              },
              '26001_params_templateId': {
                'ui:hidden': 'conditionName!=26001',
                'ui:width': '40%',
              },
              '26001_comparator': {
                'ui:hidden': 'conditionName!=26001',
                'ui:width': '15%',
              },
              '26001_return_value': {
                'ui:hidden': 'conditionName!=26001',
                'ui:width': '40%',
              },
              '27001_comparator': {
                'ui:hidden': 'conditionName!=27001',
                'ui:width': '15%',
              },
              '27001_return_value': {
                'ui:hidden': 'conditionName!=27001',
                'ui:width': '40%',
              },
              '27002_comparator': {
                'ui:hidden': 'conditionName!=27002',
                'ui:width': '15%',
              },
              '27002_return_value': {
                'ui:hidden': 'conditionName!=27002',
                'ui:width': '40%',
              },
              '35001_comparator': {
                'ui:hidden': 'conditionName!=35001',
                'ui:width': '15%',
              },
              '35001_return_value': {
                'ui:hidden': 'conditionName!=35001',
                'ui:width': '40%',
              },
              '35002_comparator': {
                'ui:hidden': 'conditionName!=35002',
                'ui:width': '15%',
              },
              '35002_return_value': {
                'ui:hidden': 'conditionName!=35002',
                'ui:width': '40%',
              },
              '36001_params_userIdList': {
                'ui:hidden': 'conditionName!=36001',
                'ui:width': '40%',
              },
              '36001_comparator': {
                'ui:hidden': 'conditionName!=36001',
                'ui:width': '15%',
              },
              '36001_return_value': {
                'ui:hidden': 'conditionName!=36001',
                'ui:width': '40%',
              },
              '59001_params_umpInstanceIds': {
                'ui:hidden': 'conditionName!=59001',
                'ui:width': '40%',
              },
              '59001_comparator': {
                'ui:hidden': 'conditionName!=59001',
                'ui:width': '15%',
              },
              '59001_return_value': {
                'ui:hidden': 'conditionName!=59001',
                'ui:width': '40%',
              },
              '59002_params_umpInstanceIds': {
                'ui:hidden': 'conditionName!=59002',
                'ui:width': '40%',
              },
              '59002_comparator': {
                'ui:hidden': 'conditionName!=59002',
                'ui:width': '15%',
              },
              '59002_return_value': {
                'ui:hidden': 'conditionName!=59002',
                'ui:width': '40%',
              },
              '60001_params_crowdId': {
                'ui:hidden': 'conditionName!=60001',
                'ui:width': '40%',
              },
              '60001_comparator': {
                'ui:hidden': 'conditionName!=60001',
                'ui:width': '15%',
              },
              '60001_return_value': {
                'ui:hidden': 'conditionName!=60001',
                'ui:width': '40%',
              },
              '97001_comparator': {
                'ui:hidden': 'conditionName!=97001',
                'ui:width': '15%',
              },
              '97001_return_value': {
                'ui:hidden': 'conditionName!=97001',
                'ui:width': '40%',
              },
              '115001_comparator': {
                'ui:hidden': 'conditionName!=115001',
                'ui:width': '15%',
              },
              '115001_return_value': {
                'ui:hidden': 'conditionName!=115001',
                'ui:width': '40%',
              },
              '116001_params_surveyId': {
                'ui:hidden': 'conditionName!=116001',
                'ui:width': '40%',
              },
              '116001_comparator': {
                'ui:hidden': 'conditionName!=116001',
                'ui:width': '15%',
              },
              '116001_return_value': {
                'ui:hidden': 'conditionName!=116001',
                'ui:width': '40%',
              },
              '117001_params_categoryIdList': {
                'ui:hidden': 'conditionName!=117001',
                'ui:width': '40%',
              },
              '117001_comparator': {
                'ui:hidden': 'conditionName!=117001',
                'ui:width': '15%',
              },
              '117001_return_value': {
                'ui:hidden': 'conditionName!=117001',
                'ui:width': '40%',
              },
              '131001_params_nodeCode': {
                'ui:hidden': 'conditionName!=131001',
                'ui:width': '40%',
              },
              '131001_comparator': {
                'ui:hidden': 'conditionName!=131001',
                'ui:width': '15%',
              },
              '131001_return_value': {
                'ui:hidden': 'conditionName!=131001',
                'ui:width': '40%',
              },
              '134001_params_umpInstanceIds': {
                'ui:hidden': 'conditionName!=134001',
                'ui:width': '40%',
              },
              '134001_comparator': {
                'ui:hidden': 'conditionName!=134001',
                'ui:width': '15%',
              },
              '134001_return_value': {
                'ui:hidden': 'conditionName!=134001',
                'ui:width': '40%',
              },
              '169001_params_surveyId': {
                'ui:hidden': 'conditionName!=169001',
                'ui:width': '40%',
              },
              '169001_comparator': {
                'ui:hidden': 'conditionName!=169001',
                'ui:width': '15%',
              },
              '169001_return_value': {
                'ui:hidden': 'conditionName!=169001',
                'ui:width': '40%',
              },
              '170002_comparator': {
                'ui:hidden': 'conditionName!=170002',
                'ui:width': '15%',
              },
              '170002_return_value': {
                'ui:hidden': 'conditionName!=170002',
                'ui:width': '40%',
              },
              '173001_params_productId': {
                'ui:hidden': 'conditionName!=173001',
                'ui:width': '40%',
              },
              '173001_params_crowdId': {
                'ui:hidden': 'conditionName!=173001',
                'ui:width': '40%',
              },
              '173001_params_token': {
                'ui:hidden': 'conditionName!=173001',
                'ui:width': '40%',
              },
              '173001_comparator': {
                'ui:hidden': 'conditionName!=173001',
                'ui:width': '15%',
              },
              '173001_return_value': {
                'ui:hidden': 'conditionName!=173001',
                'ui:width': '40%',
              },
              '178001_comparator': {
                'ui:hidden': 'conditionName!=178001',
                'ui:width': '15%',
              },
              '178001_return_value': {
                'ui:hidden': 'conditionName!=178001',
                'ui:width': '40%',
              },
              '180001_comparator': {
                'ui:hidden': 'conditionName!=180001',
                'ui:width': '15%',
              },
              '180001_return_value': {
                'ui:hidden': 'conditionName!=180001',
                'ui:width': '40%',
              },
              '181001_params_hidList': {
                'ui:hidden': 'conditionName!=181001',
                'ui:width': '40%',
              },
              '181001_comparator': {
                'ui:hidden': 'conditionName!=181001',
                'ui:width': '15%',
              },
              '181001_return_value': {
                'ui:hidden': 'conditionName!=181001',
                'ui:width': '40%',
              },
              '182001_params_sellerIdList': {
                'ui:hidden': 'conditionName!=182001',
                'ui:width': '40%',
              },
              '182001_comparator': {
                'ui:hidden': 'conditionName!=182001',
                'ui:width': '15%',
              },
              '182001_return_value': {
                'ui:hidden': 'conditionName!=182001',
                'ui:width': '40%',
              },
              '182002_params_checkInDate2': {
                'ui:hidden': 'conditionName!=182002',
                'ui:width': '40%',
              },
              '182002_comparator': {
                'ui:hidden': 'conditionName!=182002',
                'ui:width': '15%',
              },
              '182002_return_value': {
                'ui:hidden': 'conditionName!=182002',
                'ui:width': '40%',
              },
              '183001_params_checkOutDate2': {
                'ui:hidden': 'conditionName!=183001',
                'ui:width': '40%',
              },
              '183001_comparator': {
                'ui:hidden': 'conditionName!=183001',
                'ui:width': '15%',
              },
              '183001_return_value': {
                'ui:hidden': 'conditionName!=183001',
                'ui:width': '40%',
              },
              '184001_params_checkInDate2': {
                'ui:hidden': 'conditionName!=184001',
                'ui:width': '40%',
              },
              '184001_comparator': {
                'ui:hidden': 'conditionName!=184001',
                'ui:width': '15%',
              },
              '184001_return_value': {
                'ui:hidden': 'conditionName!=184001',
                'ui:width': '40%',
              },
              '185001_params_orderCreateDate2': {
                'ui:hidden': 'conditionName!=185001',
                'ui:width': '40%',
              },
              '185001_comparator': {
                'ui:hidden': 'conditionName!=185001',
                'ui:width': '15%',
              },
              '185001_return_value': {
                'ui:hidden': 'conditionName!=185001',
                'ui:width': '40%',
              },
              '186001_params_orderCreateDate2': {
                'ui:hidden': 'conditionName!=186001',
                'ui:width': '40%',
              },
              '186001_comparator': {
                'ui:hidden': 'conditionName!=186001',
                'ui:width': '15%',
              },
              '186001_return_value': {
                'ui:hidden': 'conditionName!=186001',
                'ui:width': '40%',
              },
              '187001_comparator': {
                'ui:hidden': 'conditionName!=187001',
                'ui:width': '15%',
              },
              '187001_return_value': {
                'ui:hidden': 'conditionName!=187001',
                'ui:width': '40%',
              },
              '188001_comparator': {
                'ui:hidden': 'conditionName!=188001',
                'ui:width': '15%',
              },
              '188001_return_value': {
                'ui:hidden': 'conditionName!=188001',
                'ui:width': '40%',
              },
              '189001_comparator': {
                'ui:hidden': 'conditionName!=189001',
                'ui:width': '15%',
              },
              '189001_return_value': {
                'ui:hidden': 'conditionName!=189001',
                'ui:width': '40%',
              },
              '194001_comparator': {
                'ui:hidden': 'conditionName!=194001',
                'ui:width': '15%',
              },
              '194001_return_value': {
                'ui:hidden': 'conditionName!=194001',
                'ui:width': '40%',
              },
              '200001_params_tailPaidDate3': {
                'ui:hidden': 'conditionName!=200001',
                'ui:width': '40%',
              },
              '200001_comparator': {
                'ui:hidden': 'conditionName!=200001',
                'ui:width': '15%',
              },
              '200001_return_value': {
                'ui:hidden': 'conditionName!=200001',
                'ui:width': '40%',
              },
              '201001_params_tailPaidDate2': {
                'ui:hidden': 'conditionName!=201001',
                'ui:width': '40%',
              },
              '201001_comparator': {
                'ui:hidden': 'conditionName!=201001',
                'ui:width': '15%',
              },
              '201001_return_value': {
                'ui:hidden': 'conditionName!=201001',
                'ui:width': '40%',
              },
              '207001_comparator': {
                'ui:hidden': 'conditionName!=207001',
                'ui:width': '15%',
              },
              '207001_return_value': {
                'ui:hidden': 'conditionName!=207001',
                'ui:width': '40%',
              },
              '208001_comparator': {
                'ui:hidden': 'conditionName!=208001',
                'ui:width': '15%',
              },
              '208001_return_value': {
                'ui:hidden': 'conditionName!=208001',
                'ui:width': '40%',
              },
              '212001_params_virtralCode': {
                'ui:hidden': 'conditionName!=212001',
                'ui:width': '40%',
              },
              '212001_comparator': {
                'ui:hidden': 'conditionName!=212001',
                'ui:width': '15%',
              },
              '212001_return_value': {
                'ui:hidden': 'conditionName!=212001',
                'ui:width': '40%',
              },
              '213001_params_vendor': {
                'ui:hidden': 'conditionName!=213001',
                'ui:width': '40%',
              },
              '213001_params_partnerId': {
                'ui:hidden': 'conditionName!=213001',
                'ui:width': '40%',
              },
              '213001_comparator': {
                'ui:hidden': 'conditionName!=213001',
                'ui:width': '15%',
              },
              '213001_return_value': {
                'ui:hidden': 'conditionName!=213001',
                'ui:width': '40%',
              },
              '215001_params_payTime4PointDouble2': {
                'ui:hidden': 'conditionName!=215001',
                'ui:width': '40%',
              },
              '215001_comparator': {
                'ui:hidden': 'conditionName!=215001',
                'ui:width': '15%',
              },
              '215001_return_value': {
                'ui:hidden': 'conditionName!=215001',
                'ui:width': '40%',
              },
              '216001_params_payTime4PointDouble3': {
                'ui:hidden': 'conditionName!=216001',
                'ui:width': '40%',
              },
              '216001_comparator': {
                'ui:hidden': 'conditionName!=216001',
                'ui:width': '15%',
              },
              '216001_return_value': {
                'ui:hidden': 'conditionName!=216001',
                'ui:width': '40%',
              },
              '219001_comparator': {
                'ui:hidden': 'conditionName!=219001',
                'ui:width': '15%',
              },
              '219001_return_value': {
                'ui:hidden': 'conditionName!=219001',
                'ui:width': '40%',
              },
              '220001_comparator': {
                'ui:hidden': 'conditionName!=220001',
                'ui:width': '15%',
              },
              '220001_return_value': {
                'ui:hidden': 'conditionName!=220001',
                'ui:width': '40%',
              },
              '221001_params_virtualCode': {
                'ui:hidden': 'conditionName!=221001',
                'ui:width': '40%',
              },
              '221001_comparator': {
                'ui:hidden': 'conditionName!=221001',
                'ui:width': '15%',
              },
              '221001_return_value': {
                'ui:hidden': 'conditionName!=221001',
                'ui:width': '40%',
              },
              '222001_params_virtualCode': {
                'ui:hidden': 'conditionName!=222001',
                'ui:width': '40%',
              },
              '222001_comparator': {
                'ui:hidden': 'conditionName!=222001',
                'ui:width': '15%',
              },
              '222001_return_value': {
                'ui:hidden': 'conditionName!=222001',
                'ui:width': '40%',
              },
              '223001_params_subActId': {
                'ui:hidden': 'conditionName!=223001',
                'ui:width': '40%',
              },
              '223001_params_groupResourceId': {
                'ui:hidden': 'conditionName!=223001',
                'ui:width': '40%',
              },
              '223001_comparator': {
                'ui:hidden': 'conditionName!=223001',
                'ui:width': '15%',
              },
              '223001_return_value': {
                'ui:hidden': 'conditionName!=223001',
                'ui:width': '40%',
              },
              '224001_params_virtualCode': {
                'ui:hidden': 'conditionName!=224001',
                'ui:width': '40%',
              },
              '224001_params_virtualValue': {
                'ui:hidden': 'conditionName!=224001',
                'ui:width': '40%',
              },
              '224001_comparator': {
                'ui:hidden': 'conditionName!=224001',
                'ui:width': '15%',
              },
              '224001_return_value': {
                'ui:hidden': 'conditionName!=224001',
                'ui:width': '40%',
              },
              '225001_params_subActId': {
                'ui:hidden': 'conditionName!=225001',
                'ui:width': '40%',
              },
              '225001_comparator': {
                'ui:hidden': 'conditionName!=225001',
                'ui:width': '15%',
              },
              '225001_return_value': {
                'ui:hidden': 'conditionName!=225001',
                'ui:width': '40%',
              },
              '226001_comparator': {
                'ui:hidden': 'conditionName!=226001',
                'ui:width': '15%',
              },
              '226001_return_value': {
                'ui:hidden': 'conditionName!=226001',
                'ui:width': '40%',
              },
              '229001_params_sellerId': {
                'ui:hidden': 'conditionName!=229001',
                'ui:width': '40%',
              },
              '229001_params_templateCode': {
                'ui:hidden': 'conditionName!=229001',
                'ui:width': '40%',
              },
              '229001_comparator': {
                'ui:hidden': 'conditionName!=229001',
                'ui:width': '15%',
              },
              '229001_return_value': {
                'ui:hidden': 'conditionName!=229001',
                'ui:width': '40%',
              },
              '230002_params_userIdList': {
                'ui:hidden': 'conditionName!=230002',
                'ui:width': '40%',
              },
              '230002_comparator': {
                'ui:hidden': 'conditionName!=230002',
                'ui:width': '15%',
              },
              '230002_return_value': {
                'ui:hidden': 'conditionName!=230002',
                'ui:width': '40%',
              },
              '231001_params_templateId': {
                'ui:hidden': 'conditionName!=231001',
                'ui:width': '40%',
              },
              '231001_comparator': {
                'ui:hidden': 'conditionName!=231001',
                'ui:width': '15%',
              },
              '231001_return_value': {
                'ui:hidden': 'conditionName!=231001',
                'ui:width': '40%',
              },
              '232001_params_subActIdList': {
                'ui:hidden': 'conditionName!=232001',
                'ui:width': '40%',
              },
              '232001_comparator': {
                'ui:hidden': 'conditionName!=232001',
                'ui:width': '15%',
              },
              '232001_return_value': {
                'ui:hidden': 'conditionName!=232001',
                'ui:width': '40%',
              },
              '234001_comparator': {
                'ui:hidden': 'conditionName!=234001',
                'ui:width': '15%',
              },
              '234001_return_value': {
                'ui:hidden': 'conditionName!=234001',
                'ui:width': '40%',
              },
              '234002_comparator': {
                'ui:hidden': 'conditionName!=234002',
                'ui:width': '15%',
              },
              '234002_return_value': {
                'ui:hidden': 'conditionName!=234002',
                'ui:width': '40%',
              },
              '235001_comparator': {
                'ui:hidden': 'conditionName!=235001',
                'ui:width': '15%',
              },
              '235001_return_value': {
                'ui:hidden': 'conditionName!=235001',
                'ui:width': '40%',
              },
              '235002_comparator': {
                'ui:hidden': 'conditionName!=235002',
                'ui:width': '15%',
              },
              '235002_return_value': {
                'ui:hidden': 'conditionName!=235002',
                'ui:width': '40%',
              },
              '236001_comparator': {
                'ui:hidden': 'conditionName!=236001',
                'ui:width': '15%',
              },
              '236001_return_value': {
                'ui:hidden': 'conditionName!=236001',
                'ui:width': '40%',
              },
              '237001_comparator': {
                'ui:hidden': 'conditionName!=237001',
                'ui:width': '15%',
              },
              '237001_return_value': {
                'ui:hidden': 'conditionName!=237001',
                'ui:width': '40%',
              },
              '238001_comparator': {
                'ui:hidden': 'conditionName!=238001',
                'ui:width': '15%',
              },
              '238001_return_value': {
                'ui:hidden': 'conditionName!=238001',
                'ui:width': '40%',
              },
              '239001_comparator': {
                'ui:hidden': 'conditionName!=239001',
                'ui:width': '15%',
              },
              '239001_return_value': {
                'ui:hidden': 'conditionName!=239001',
                'ui:width': '40%',
              },
              '240001_comparator': {
                'ui:hidden': 'conditionName!=240001',
                'ui:width': '15%',
              },
              '240001_return_value': {
                'ui:hidden': 'conditionName!=240001',
                'ui:width': '40%',
              },
              '241001_params_subActTemplatedId': {
                'ui:hidden': 'conditionName!=241001',
                'ui:width': '40%',
              },
              '241001_comparator': {
                'ui:hidden': 'conditionName!=241001',
                'ui:width': '15%',
              },
              '241001_return_value': {
                'ui:hidden': 'conditionName!=241001',
                'ui:width': '40%',
              },
              '244001_params_templateId': {
                'ui:hidden': 'conditionName!=244001',
                'ui:width': '40%',
              },
              '244001_comparator': {
                'ui:hidden': 'conditionName!=244001',
                'ui:width': '15%',
              },
              '244001_return_value': {
                'ui:hidden': 'conditionName!=244001',
                'ui:width': '40%',
              },
              '245001_params_whiteItemIdList': {
                'ui:hidden': 'conditionName!=245001',
                'ui:width': '40%',
              },
              '245001_comparator': {
                'ui:hidden': 'conditionName!=245001',
                'ui:width': '15%',
              },
              '245001_return_value': {
                'ui:hidden': 'conditionName!=245001',
                'ui:width': '40%',
              },
              '250001_params_templateIds': {
                'ui:hidden': 'conditionName!=250001',
                'ui:width': '40%',
              },
              '250001_comparator': {
                'ui:hidden': 'conditionName!=250001',
                'ui:width': '15%',
              },
              '250001_return_value': {
                'ui:hidden': 'conditionName!=250001',
                'ui:width': '40%',
              },
              '251001_params_tpOrderList': {
                'ui:hidden': 'conditionName!=251001',
                'ui:width': '40%',
              },
              '251001_comparator': {
                'ui:hidden': 'conditionName!=251001',
                'ui:width': '15%',
              },
              '251001_return_value': {
                'ui:hidden': 'conditionName!=251001',
                'ui:width': '40%',
              },
              '251002_params_whiteOrderPayTime': {
                'ui:hidden': 'conditionName!=251002',
                'ui:width': '40%',
              },
              '251002_params_whiteCategoryList': {
                'ui:hidden': 'conditionName!=251002',
                'ui:width': '40%',
              },
              '251002_comparator': {
                'ui:hidden': 'conditionName!=251002',
                'ui:width': '15%',
              },
              '251002_return_value': {
                'ui:hidden': 'conditionName!=251002',
                'ui:width': '40%',
              },
              '252001_params_whiteItemTags': {
                'ui:hidden': 'conditionName!=252001',
                'ui:width': '40%',
              },
              '252001_comparator': {
                'ui:hidden': 'conditionName!=252001',
                'ui:width': '15%',
              },
              '252001_return_value': {
                'ui:hidden': 'conditionName!=252001',
                'ui:width': '40%',
              },
              '253001_comparator': {
                'ui:hidden': 'conditionName!=253001',
                'ui:width': '15%',
              },
              '253001_return_value': {
                'ui:hidden': 'conditionName!=253001',
                'ui:width': '40%',
              },
              '255001_comparator': {
                'ui:hidden': 'conditionName!=255001',
                'ui:width': '15%',
              },
              '255001_return_value': {
                'ui:hidden': 'conditionName!=255001',
                'ui:width': '40%',
              },
              '257001_comparator': {
                'ui:hidden': 'conditionName!=257001',
                'ui:width': '15%',
              },
              '257001_return_value': {
                'ui:hidden': 'conditionName!=257001',
                'ui:width': '40%',
              },
              '258001_params_bizType': {
                'ui:hidden': 'conditionName!=258001',
                'ui:width': '40%',
              },
              '258001_comparator': {
                'ui:hidden': 'conditionName!=258001',
                'ui:width': '15%',
              },
              '258001_return_value': {
                'ui:hidden': 'conditionName!=258001',
                'ui:width': '40%',
              },
              '259001_comparator': {
                'ui:hidden': 'conditionName!=259001',
                'ui:width': '15%',
              },
              '259001_return_value': {
                'ui:hidden': 'conditionName!=259001',
                'ui:width': '40%',
              },
              '261001_params_templateIds': {
                'ui:hidden': 'conditionName!=261001',
                'ui:width': '40%',
              },
              '261001_comparator': {
                'ui:hidden': 'conditionName!=261001',
                'ui:width': '15%',
              },
              '261001_return_value': {
                'ui:hidden': 'conditionName!=261001',
                'ui:width': '40%',
              },
              '271001_comparator': {
                'ui:hidden': 'conditionName!=271001',
                'ui:width': '15%',
              },
              '271001_return_value': {
                'ui:hidden': 'conditionName!=271001',
                'ui:width': '40%',
              },
              '273001_comparator': {
                'ui:hidden': 'conditionName!=273001',
                'ui:width': '15%',
              },
              '273001_return_value': {
                'ui:hidden': 'conditionName!=273001',
                'ui:width': '40%',
              },
              '281001_params_templateId': {
                'ui:hidden': 'conditionName!=281001',
                'ui:width': '40%',
              },
              '281001_comparator': {
                'ui:hidden': 'conditionName!=281001',
                'ui:width': '15%',
              },
              '281001_return_value': {
                'ui:hidden': 'conditionName!=281001',
                'ui:width': '40%',
              },
              '285001_params_crowdIds': {
                'ui:hidden': 'conditionName!=285001',
                'ui:width': '40%',
              },
              '285001_comparator': {
                'ui:hidden': 'conditionName!=285001',
                'ui:width': '15%',
              },
              '285001_return_value': {
                'ui:hidden': 'conditionName!=285001',
                'ui:width': '40%',
              },
              '286001_params_crowdIds': {
                'ui:hidden': 'conditionName!=286001',
                'ui:width': '40%',
              },
              '286001_comparator': {
                'ui:hidden': 'conditionName!=286001',
                'ui:width': '15%',
              },
              '286001_return_value': {
                'ui:hidden': 'conditionName!=286001',
                'ui:width': '40%',
              },
              '289001_comparator': {
                'ui:hidden': 'conditionName!=289001',
                'ui:width': '15%',
              },
              '289001_return_value': {
                'ui:hidden': 'conditionName!=289001',
                'ui:width': '40%',
              },
            },
          },
          actionId: {
            'ui:hidden': "@formData.wfCommonInfo.productCode=='ACCUMULATED'",
          },
        },
      },
    },
    applyGiftPackageShowConfig: {
      template: {
        'ui:disabled': '@formData.wfCommonInfo.theTwoEditors===true',
      },
      giftPkgDescription: { 'ui:hidden': "template!='1' && template!='3'" },
      fixedAmountBefore: {
        'ui:options': { addonBefore: '满', addonAfter: '元' },
        'ui:hidden': "faceValue!='1' || (template!=1 && template!=4)",
        'ui:width': '42%',
      },
      fixedAmountAfter: {
        'ui:options': { addonBefore: '减', addonAfter: '元' },
        'ui:hidden': "faceValue!='1'",
        'ui:width': '42%',
      },
      discount: {
        'ui:options': { addonAfter: '折' },
        'ui:hidden': "faceValue!='2'",
        'ui:width': '50%',
      },
      reduce: {
        'ui:options': { addonAfter: '元' },
        'ui:hidden': "faceValue!='3'",
        'ui:width': '50%',
      },
      faceValuePurePrefixText: {
        'ui:hidden': "faceValue!='5'",
        'ui:width': '50%',
      },
      faceValuePureInfixText: {
        'ui:hidden':
          "{{(!(rootValue.faceValue=='4' && rootValue.template==2)) && (!(rootValue.faceValue=='5'))}}",
        'ui:width': '50%',
      },
      faceValuePureText: {
        'ui:hidden': "faceValue!='4' && faceValue!='5'",
        'ui:width': '50%',
      },
      effectiveTimeType: { 'ui:hidden': 'template!=1 && template!=4' },
      beginTime: {
        'ui:widget': 'date',
        'ui:hidden': "effectiveTimeType!='1' || (template!=1 && template!=4)",
        'ui:width': '50%',
      },
      endTime: {
        'ui:widget': 'date',
        'ui:hidden': "effectiveTimeType!='1' || (template!=1 && template!=4)",
        'ui:width': '50%',
      },
      effectiveMin: {
        'ui:options': { addonAfter: '分钟' },
        'ui:hidden': "effectiveTimeType!='2' || (template!=1 && template!=4)",
        'ui:width': '50%',
      },
      effectiveDay: {
        'ui:options': { addonAfter: '天' },
        'ui:hidden': "effectiveTimeType!='3' || (template!=1 && template!=4)",
        'ui:width': '50%',
      },
      effectTimePureText: {
        'ui:hidden': "effectiveTimeType!='4' || template!=1",
      },
      notAvailableJumpurl: { 'ui:hidden': 'template!=3' },
      outOfStockJumpUrl: { 'ui:hidden': 'template!=3' },
      subscriptText: {
        'ui:options': { placeholder: '如：F3可领' },
        'ui:hidden': 'template!=2',
      },
      doubleCheckPointText: {
        'ui:options': { placeholder: '如：你确认花费x里程参与抽奖(兑换)吗？' },
        'ui:hidden':
          "@['LOTTERY','EXCHANGE'].indexOf(formData.wfCommonInfo.productCode)===-1 || rootValue.template!=2",
      },
      useRestrictionsText: {
        'ui:options': { placeholder: '如：满3000可用，仅F3会员可用' },
        'ui:hidden': 'template!=2 && template!=4',
      },
      wonToastText: {
        'ui:hidden':
          "@['LOTTERY'].indexOf(formData.wfCommonInfo.productCode)===-1 || rootValue.template!=2",
      },
      wonToastBtnJumpUrl: {
        'ui:hidden':
          "@['LOTTERY'].indexOf(formData.wfCommonInfo.productCode)===-1 || rootValue.template!=2",
      },
      notWonToastText: {
        'ui:hidden':
          "@['LOTTERY'].indexOf(formData.wfCommonInfo.productCode)===-1 || rootValue.template!=2",
      },
      notWonToastBtnJumpUrl: {
        'ui:hidden':
          "@['LOTTERY'].indexOf(formData.wfCommonInfo.productCode)===-1 || rootValue.template!=2",
      },
      couponRingType: { 'ui:hidden': 'template!=4' },
      couponRingValue: { 'ui:hidden': 'template!=4' },
      alipayCouponStockActChannels: { 'ui:hidden': 'template!=4' },
      alipayCouponStockBusinessLine: { 'ui:hidden': 'template!=4' },
      alipayCouponStockDeliveryCityCode: { 'ui:hidden': 'template!=4' },
      kvConfig: { 'ui:widget': 'kvList' },
    },
    applyButtonShowConfig: {
      activeNotReachedStartTime: { 'ui:hidden': 'showAdvancedSetting==false' },
      activeIsOver: { 'ui:hidden': 'showAdvancedSetting==false' },
      notAvailable: { 'ui:hidden': 'showAdvancedSetting==false' },
      available: { 'ui:hidden': 'showAdvancedSetting==false' },
      received: { 'ui:hidden': 'showAdvancedSetting==false' },
      outOfStock: { 'ui:hidden': 'showAdvancedSetting==false' },
      toUse: { 'ui:hidden': 'showAdvancedSetting==false' },
      notWonAndNotAvailable: {
        'ui:hidden':
          "@['LOTTERY'].indexOf(formData.wfCommonInfo.productCode)===-1 || rootValue.showAdvancedSetting!=true",
      },
      userParticipateLimitReached: {
        'ui:hidden': 'showAdvancedSetting==false',
      },
    },
    applyPromptConfig: {
      applySuccessMainTitle: { 'ui:hidden': 'showAdvancedSetting==false' },
      applyFailedMainTitle: { 'ui:hidden': 'showAdvancedSetting==false' },
      applySuccess: { 'ui:hidden': 'showAdvancedSetting==false' },
      lotteryNotWon: { 'ui:hidden': true },
      activeNotReachedStartTime: { 'ui:hidden': 'showAdvancedSetting==false' },
      activityHasExpired: { 'ui:hidden': 'showAdvancedSetting==false' },
      outOfStock: { 'ui:hidden': 'showAdvancedSetting==false' },
      userTotalParticipateLimitReached: {
        'ui:hidden': 'showAdvancedSetting==false',
      },
      userDailyParticipateLimitReached: {
        'ui:hidden': 'showAdvancedSetting==false',
      },
      userWeeklyParticipateLimitReached: {
        'ui:hidden': 'showAdvancedSetting==false',
      },
      userTotalLotteryWonLimitReached: { 'ui:hidden': true },
      userDailyLotteryWonLimitReached: { 'ui:hidden': true },
      userWeeklyLotteryWonLimitReached: { 'ui:hidden': true },
      userGroupLotteryWonLimitReached: {
        'ui:hidden': 'showAdvancedSetting==false',
      },
      securityBlock: { 'ui:hidden': 'showAdvancedSetting==false' },
      participationConditionsNotReached: {
        'ui:hidden': 'showAdvancedSetting==false',
      },
      deductPropertyLacking: { 'ui:hidden': true },
      bottomPrompt: { 'ui:hidden': 'showAdvancedSetting==false' },
    },
    generalExt: { kvConfig: { 'ui:widget': 'kvList' } },
  },
  formData: {
    wfCommonInfo: {
      productCode: 'AWARD',
      bizCode: 'common_award',
      theTwoEditors: false,
      subActStatus: 0,
    },
    common: {
      description: '',
      priority: '123',
      actTypeDesc: '发奖玩法',
      businessLineDesc: '公共',
      subActId: 68283,
      createdBy: '泓乐',
      businessLine: 'COMMON',
      name: '测试发奖',
      actType: 'AWARD',
      modifiedBy: '泓乐',
      beginTime: 1610627731000,
      endTime: 1612096532000,
      status: 0,
    },
  },
};
